...
УжеЗаписалиГТИН=Новый Массив;
			...
			Тело="{""filters"":[{""operator"":""="",""column"":""owner"",""filterTerm"":"""+Объект.Организация.ИНН+"""}],""sorts"":[],""pageSize"":"+"10"+",""currentPage"":"+Сместить+"}";
...
Функция ЗагрузитьИнфуВРегистр(ИсключаемИзЗапроса=Неопределено)
	...
	Запрос.Текст = 
		...
		|	0
		|ИЗ
		|	РегистрНакопления.ОстаткиКМЭмитированые_ХамелеонЦРПТ.Остатки(, Организация = &Организация И СерийныйНомер В (&cis)) КАК ОстаткиКМЭмитированые_ХамелеонЦРПТОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	ОстаткиКМЭмитированые_ХамелеонЦРПТОстатки.СерийныйНомер,
		|	ОстаткиКМЭмитированые_ХамелеонЦРПТОстатки.Продукция
		...
		|ИЗ
		|	РегистрНакопления.Резервы_ХамелеонЦРПТ.Остатки(, Организация = &Организация И СерийныйНомер В (&cis)) КАК Резервы_ХамелеонЦРПТОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	Резервы_ХамелеонЦРПТОстатки.СерийныйНомер
		|ИТОГИ
	 ...
	 КонецПопытки;
		...
		////			ПервуюПропустить=Ложь;
			...
			//	ИсключаемИзЗапроса.Добавить(Стр_1["cis"]);				
				...
				//	Добав_1.КоличествоКМ_выбыло=1;
				//КонецЕсли;
				//Добав.СтатусСтроки=Стр_1.emissionType;
...
Процедура ЗаполнитьОстаткиИз1С_1(ИсключаемИзЗапроса)
	...
	Запрос.Текст = 
		...
		|	0 КАК Резерв
		|ИЗ
		|	РегистрНакопления.ОстаткиКМ_в_Обороте_ХамелеонЦРПТ.Остатки(, Организация = &Организация И (НЕ СерийныйНомер В (&cis))) КАК ОстаткиКМ_в_Обороте_ХамелеонЦРПТОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	ОстаткиКМ_в_Обороте_ХамелеонЦРПТОстатки.СерийныйНомер,
		|	ОстаткиКМ_в_Обороте_ХамелеонЦРПТОстатки.Продукция
		...
		|	0
		|ИЗ
		|	РегистрНакопления.ОстаткиКМЭмитированые_ХамелеонЦРПТ.Остатки(, Организация = &Организация И (НЕ СерийныйНомер В (&cis))) КАК ОстаткиКМЭмитированые_ХамелеонЦРПТОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	ОстаткиКМЭмитированые_ХамелеонЦРПТОстатки.СерийныйНомер,
		|	ОстаткиКМЭмитированые_ХамелеонЦРПТОстатки.Продукция
		...
		|	СУММА(Резервы_ХамелеонЦРПТОстатки.КоличествоОстаток)
		|ИЗ
		|	РегистрНакопления.Резервы_ХамелеонЦРПТ.Остатки(, Организация = &Организация И (НЕ СерийныйНомер В (&cis))) КАК Резервы_ХамелеонЦРПТОстатки
		|
		|СГРУППИРОВАТЬ ПО
		|	Резервы_ХамелеонЦРПТОстатки.СерийныйНомер
		|ИТОГИ
