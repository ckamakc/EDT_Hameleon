...
Процедура ОтправитьАгрегациюЧереСУЗ(Thumbprint_1)
	...
	//Иначе
		...
		ТекстСохранить=ТекстСохранить+"api/aggregation";
			Иначе                                      
		HTTPЗапрос=Новый HTTPЗапрос("api/v2/"+Thumbprint_1.СайтКСУЗ+"/aggregation?omsId="+Thumbprint_1.ИдентификаторСУЗ);
		ТекстСохранить=ТекстСохранить+"api/v2/"+Thumbprint_1.СайтКСУЗ+"/aggregation?omsId="+Thumbprint_1.ИдентификаторСУЗ;
	КонецЕсли;
				...
				Если Ответ.КодСостояния=200 Тогда
					...
					МассивКМ=Новый Массив;
							...
							Если NNN[Штрих]<>Неопределено тогда
								...
								Для Каждого Стр_К Из NNN[Штрих]["children"] Цикл
...
&НаКлиенте
Процедура ПолучитьИнформациюПоКМ(Команда)
	// Вставить содержимое обработчика.
	Thumbprint_1=ПолучитьИзПользователя();	
