...
Процедура Заполним10(NNN,ID,emis)	
...
//								 uit=Стр_1["uti"];
				...
				СпрН=ВернутьСправочникЦИС(Стр_1["cis"]);
				Добав_1.cis=СпрН;
...
Процедура Заполним9(NNN,ID,emis)
Для Каждого Стр_1 Из NNN.results Цикл
		//		Если ПервуюПропустить=Истина Тогда
		////			ПервуюПропустить=Ложь;
			//		Продолжить;
	...
	КонецЦикла;
КонецПроцедуры
Процедура Заполим8(NNN,ID,emis)
Для Каждого Стр_1 Из NNN["results"] Цикл
		//		Если ПервуюПропустить=Истина Тогда
		////			ПервуюПропустить=Ложь;
			//		Продолжить;
...
//								 uit=Стр_1["uti"];
	...
	КонецЦикла;
КонецПроцедуры
	Процедура Заполим7(NNN,ID,emis)
Для Каждого Стр_1 Из NNN.results Цикл
		//		Если ПервуюПропустить=Истина Тогда
		////			ПервуюПропустить=Ложь;
			//		Продолжить;
	...
	КонецЦикла;
КонецПроцедуры
Процедура Заполним6(NNN,ID,emis)
Для Каждого Стр_1 Из NNN["results"] Цикл
		//		Если ПервуюПропустить=Истина Тогда
		////			ПервуюПропустить=Ложь;
			//		Продолжить;
	...
	КонецЦикла;
КонецПроцедуры
Процедура Заполним5(NNN,ID,emis)
Для Каждого Стр_1 Из NNN.results Цикл
		//		Если ПервуюПропустить=Истина Тогда
		////			ПервуюПропустить=Ложь;
			//		Продолжить;
	...
	КонецЦикла;
КонецПроцедуры
Процедура Заполним4(NNN,ИсключаемИзЗапроса,emis,ID)
Для Каждого Стр_1 Из NNN["results"] Цикл
		//		Если ПервуюПропустить=Истина Тогда
		////			ПервуюПропустить=Ложь;
			//		Продолжить;
			...
			Если Стр_1["uit"]<>Неопределено Тогда
				...
				СпрН=ВернутьСправочникЦИС(Стр_1["cis"]);
				Если Объект.СерийныеНомера.НайтиСтроки(Новый Структура("cis",СпрН)).Количество()>0 
					ИЛИ ИсключаемИзЗапроса.НайтиПоЗначению(СпрН)<>Неопределено
					Тогда
					Продолжить;
				КонецЕсли;
...
&НаСервереБезКонтекста
Функция   ВернутьСправочникЦИС(cis)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	КМДляКорректировки_ХамелеонЦРПТ.Ссылка,
		|	КМДляКорректировки_ХамелеонЦРПТ.Наименование
		|ИЗ
		|	Справочник.КМДляКорректировки_ХамелеонЦРПТ КАК КМДляКорректировки_ХамелеонЦРПТ
		|ГДЕ
		|	КМДляКорректировки_ХамелеонЦРПТ.Наименование = &Наименование";
	
	Запрос.УстановитьПараметр("Наименование", cis);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Наименование=cis Тогда
			СпрН=ВыборкаДетальныеЗаписи.Ссылка;
			Прервать;
		Конецесли;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(СпрН) тогда
		СпрН=справочники.КМДляКорректировки_ХамелеонЦРПТ.СоздатьЭлемент();
		СпрН.Наименование=cis ;
		СпрН.Записать();
	Конецесли;
	Возврат СпрН.Ссылка
КонецФункции

Процедура Заполним3(entries)
Для Каждого Стр_1 ИЗ entries Цикл
												...
												Конеццикла;
КонецПроцедуры
Процедура Заполним2(entries,МассивУпаковок)
	Для Каждого Стр_1 ИЗ entries Цикл
		     					//{{КОНСТРУКТОР_ЗАПРОСА_С_ОБРАБОТКОЙ_РЕЗУЛЬТАТА
	// Данный фрагмент построен конструктором.
	// При повторном использовании конструктора, внесенные вручную изменения будут утеряны!!!
	
	СпрН=ВернутьСправочникЦИС(Стр_1.id);
	
		
													Если Объект.СерийныеНомера.НайтиСтроки(Новый Структура("cis",СпрН)).Количество()>0 Тогда
														Продолжить;
													КонецЕсли;
													//Если     УжеЗаписалиГТИН.Найти(Стр_1.gtin)=Неопределено Тогда
...
Процедура Заполним1(entries)
	...
	//}}КОНСТРУКТОР_ЗАПРОСА_С_ОБРАБОТКОЙ_РЕЗУЛЬТАТА
		...
		//СпрН=Справочники.КМДляКорректировки_ХамелеонЦРПТ.НайтиПоНаименованию(Стр_1.cis,Истина);
...
УжеЗаписалиГТИН=Новый Массив;
			...
			КонецЕсли;
											...
											Если NNN.Свойство("entries",entries) Тогда
												Заполним2(entries,МассивУпаковок)
											КонецЕсли;
Тотал=NNN.Total;
Смещение=Смещение+1;
...
Процедура УстановитьПараметр()
			...
			Для Каждого стр_1 Из СписокСерийников.Отбор.Элементы Цикл
			...
			Для Каждого Стр Из МассивУдалимначалаВсе Цикл
				...
				СписокСерийников.Отбор.Элементы.Удалить(Стр);
			Конеццикла;
			
			Стр=СписокСерийников.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			Стр.ВидСравнения=ВидСравненияКомпоновкиДанных.Равно;
			Стр.Использование=Истина;
			Стр.ЛевоеЗначение=Новый ПолеКомпоновкиДанных("ДокументКорректировки");
...
Функция ЗагрузитьИнфуВРегистр(ИсключаемИзЗапроса=Неопределено)
	...
	Масс=Новый СписокЗначений;
	//Серийники.ВыгрузитьКолонку("cis");
	
	Для Каждого Стр_1 Из Серийники Цикл
		Масс.Добавить(Стр_1.cis.Наименование);
	...
	Пока ВыборкаПродукция.Следующий() Цикл
		...
		Пока ВыборкаСерийныйНомер.Следующий() Цикл
			...
			СпрН=ВернутьСправочникЦИС(ВыборкаСерийныйНомер.СерийныйНомер);
			
			НайтиСтр=Серийники.НайтиСтроки(Новый Структура("cis",СпрН));
...
//				Добав.cis=СпрН;
	...
	Если Серийники.Количество()>0 ТОгда
		РегДв=РегистрыСведений.СерийныеНомераСправочник_ХамелеонЦРПТ.СоздатьНаборЗаписей();
...
//	ДатаНач=КонецДня(датаНач)+1;
 ...
 Исключение
	 КонецПопытки;
			...
			emis="";
			       ID="";
			Заполним4(NNN,ИсключаемИзЗапроса,emis,ID);	
	Иначе
...
Процедура ЗаполнитьОстаткиИз1С_1(ИсключаемИзЗапроса)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	НомераКМ_ХамелеонЦРПТ.cis.Наименование Как cis
		|ИЗ
		|	РегистрСведений.СерийныеНомераСправочник_ХамелеонЦРПТ КАК НомераКМ_ХамелеонЦРПТ
		|ГДЕ
...
Исключение
 ...
 КонецПопытки;
		...
		Если 			Соответствие=Ложь Тогда
			...
			Заполим7(NNN,ID,emis)	
		Иначе
	Заполим8(NNN,ID,emis)
...
Исключение
 ...
 КонецПопытки;
		...
		Если 			Соответствие=Ложь Тогда
			Заполним9(NNN,ID,emis);	
			
		Иначе
	
	        	Заполним10(NNN,ID,emis);
	...
	Пока ВыборкаПродукция.Следующий() Цикл
		...
		Пока ВыборкаСерийныйНомер.Следующий() Цикл
			//Если ИсключаемИзЗапроса.НайтиПоЗначению(ВыборкаСерийныйНомер.СерийныйНомер)=Неопределено Тогда
				...
				Добав_1=СюдаДобавить.Найти(СпрН,"cis");
				Если   Добав_1=Неопределено Тогда
						//ИсключаемИзЗапроса.Добавить(ВыборкаСерийныйНомер.СерийныйНомер);
						Добав_1=СюдаДобавить.Добавить();
				Конецесли;
...
Процедура HELPНаСервере(Теккк)
	// Вставить содержимое обработчика.
	Мак=Документы.АвтоКорректировкаОстатковСправочник_ХамелеонЦРПТ.ПолучитьМакет("HELP");
	Теккк=Мак;
КонецПроцедуры
