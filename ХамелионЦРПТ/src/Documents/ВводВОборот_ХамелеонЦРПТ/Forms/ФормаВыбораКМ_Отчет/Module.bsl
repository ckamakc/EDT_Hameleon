	...
	Если ТекущаяСтраница=Элементы.ВыборПартии Тогда
		Если Элементы.Продукция.ТекущиеДанные<>Неопределено Тогда
			...
			Для Каждого Стр Из МассивУдалимначалаВсе Цикл
				Если стр.Представление="ОТБОРНИЖНИЙ" Тогда
					Продолжить;
				Конецесли;
			...
			Элементы.ВыборПартии.Видимость=Истина
				
		КонецЕслИ;
	Иначе
		//Элементы.ВыборПартии.Видимость=Ложь
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	GTIN="";
	Параметры.Свойство("GTIN",GTIN);
	Если ЗначениеЗаполнено(GTIN) Тогда
		Стр=Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		Стр.ВидСравнения=ВидСравненияКомпоновкиДанных.Содержит;
		Стр.Использование=Истина;
		Стр.ЛевоеЗначение=Новый ПолеКомпоновкиДанных("СерийныйНомер");
		Стр.ПравоеЗначение=GTIN; 
		Стр.Представление="ОТБОРНИЖНИЙ";
		Стр.РежимОтображения=РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ;
	Конецесли;
...
Процедура Выбор(Команда)
	...
	Элементы.Страницы.ТекущаяСтраница=Элементы.ВыборПартии;
	СтраницыПриСменеСтраницы(Неопределено, Элементы.ВыборПартии)

КонецПроцедуры

&НаКлиенте
Процедура Расформировать(Команда)
	// Вставить содержимое обработчика.
...
Функция СОздатьДокументРасформирования(Знач ШШ,Знач Пользователь,Знач Организация)
	...
	Докум.СтатусКМ=Перечисления.СтатусыКМДляАгрегации_ХамелеонЦРПТ.КМЭмитирован;
