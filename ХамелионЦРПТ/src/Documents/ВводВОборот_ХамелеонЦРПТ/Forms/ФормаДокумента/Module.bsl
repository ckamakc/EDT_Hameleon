...
ЗапретОтправкиОтчетаОНанесенииБудущейДатой=РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ЗапретОтправкиОтчетаОНанесенииБудущейДатой;	
НеПроверятьБалансПриОправкеОтчетаОНанесении=РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().НеПроверятьБалансПриОправкеОтчетаОНанесении;	
...
Объект.Транзакция = НашеОписание.ПривестиЗначение(Объект.Транзакция); 
	...
	НовыйЭлемент = Элементы.Добавить("ДокОснование",Тип("ПолеФормы"),Элементы.НомерДта);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
	НовыйЭлемент.Шрифт = Новый Шрифт(,,Истина);
	НовыйЭлемент.ПутьКДанным = "Объект.ДокОснование";
	
	//uncommit
	ФТО_РаботаСФормами.ДобавитьКомандуФормы(ЭтаФорма,"ВыбратьВСД","КомандаВыбратьВСД","Выбрать ВСД",ВидКнопкиФормы.ОбычнаяКнопка, Элементы.Товары.КоманднаяПанель,Элементы.Товары.КоманднаяПанель.ПодчиненныеЭлементы.Подсчитать);
	...
	ГруппаФормы = ФТО_РаботаСФормами.ДобавитьГруппуФормы(ЭтаФорма,"МВ_ГруппаПодвала","", ЭтаФорма);
	ГруппаФормы.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяЕслиВозможно;
	ФТО_РаботаСФормами.ДобавитьПолеФормы(ЭтаФорма,"МВ_ИмяФайла","Объект.МВ_ИмяФайла","Имя файла" ,ВидПоляФормы.ПолеВвода,ГруппаФормы);
	ФТО_РаботаСФормами.ДобавитьПолеФормы(ЭтаФорма,"МВ_ДатаФактическогоПроизводства","Объект.МВ_ДатаФактическогоПроизводства","Факт. производство" ,ВидПоляФормы.ПолеВвода,ГруппаФормы);

	//Элементы.МВ_ОтчетАрлеОтправлен.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	//ИмяКоманды="МВ_ВыбратьВСД";	
	//КомандаФормы = Команды.Добавить(ИмяКоманды);
	//КомандаФормы.Действие = "КомандаВыбратьВСД";
	//КомандаФормы.Заголовок = "Выбрать ВСД";
	//КомандаФормы.ИзменяетСохраняемыеДанные = Ложь;
	//КомандаФормы.Отображение = ОтображениеКнопки.Текст;
	//НовыйЭлемент = Элементы.Добавить("ВыбратьВСД"+ ИмяКоманды, Тип("КнопкаФормы"), Элементы.Товары.КоманднаяПанель,Элементы.Товары.КоманднаяПанель.ПодчиненныеЭлементы.Подсчитать);
	//НовыйЭлемент.Вид = ВидКнопкиФормы.ОбычнаяКнопка;
	//НовыйЭлемент.Заголовок = "Выбрать ВСД";
	...
	Иначе
		...
		ИначеЕсли ОБъект.ТипДокумента=Перечисления.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ВыводИзОборота Тогда
			...
			Если Объект.ТекущийПользователь.СайтыВходаВСистему.Наименование="Альтернативный табак"
				...
				Тогда
				Элементы.ПричинаВыводаИзОборота_Табак.Видимость=Истина;
				Элементы.ПричинаНеТабак.Видимость=ложь;
				Элементы.ВидПерввичногоДокумента.Видимость=Ложь;
				Элементы.РегистрационныйНомерККТ.Видимость=Ложь;
			Иначе
				Элементы.ВидПерввичногоДокумента.Видимость=Истина;
				Элементы.ПричинаНеТабак.Видимость=Истина;
				Элементы.ПричинаВыводаИзОборота_Табак.Видимость=ложь;
				Если Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.OTHER Тогда
					Объект.ВидПервичногоДокумента=Перечисления.ТипПервичногоДокумента_ХамелеонЦРПТ.OTHER;
				Конецесли;
				
				Элементы.ПричинаВыводаИзОборота_Строка.Видимость=Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.OTHER;
				
				Элементы.РегистрационныйНомерККТ.Видимость=Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.RETAIL;
				          
				Элементы.ВидПервичногоДокумента4.Видимость=Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.REMOTE_SALE;
				Элементы.ВидПервичногоДокумента.Видимость=Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.RETAIL;
				Элементы.ВидПервичногоДокумента1.Видимость=Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.DAMAGE_LOSS ИЛИ
				Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.DESTRUCTION ИЛИ
				Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.ENTERPRISE_USE
				;
				Элементы.ВидПервичногоДокумента2.Видимость=Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.BEYOND_EEC_EXPORT;
				Элементы.ВидПервичногоДокумента3.Видимость=Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.EEC_EXPORT;
...
&НаКлиенте
Процедура ПечатьЭтикеток_ЦРПТ(Команда)
	// Вставить содержимое обработчика.
КонецПроцедуры
...
//-- МВ:Овчинников 24.06.2021
Функция СформироватьОтчетОбИспользованииМассивДляАрлы(Thumbprint_1)
...
Конецфункции//-- МВ:Овчинников 24.06.2021
...
Процедура ОтправитьОтчетОбИспользовании(Thumbprint_1,УжеОтправили=Ложь)
	...
	текстСохранить=текстСохранить+"api/v2/"+Thumbprint_1.СайтКСУЗ+"/utilisation?omsId="+Thumbprint_1.ИдентификаторСУЗ;
...
КонецПопытки;
		...
		Сч=СЧ+1;
	    текстСохранить=текстСохранить+Символы.ПС+Символы.ПС+Формат(Сч,"ЧГ=")+" запрос отправляем";	
...
//		КонецЕсли;
	...
	Если ЗначениеЗаполнено(Подписанный) тогда
		...
		текстСохранить=текстСохранить+символы.ПС+"X-Signature:"+Сред(Подписанный,1,100);
	КонецЕсли;
	текстСохранить=текстСохранить+Символы.ПС+"Тело запроса";
...
//Для Каждого Стр Из Объект.Товары Цикл
	...
	//		Чтен.ЗаписатьЗначение("NO_RETAIL_USE");
			...
			Если Объект.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.OTHER Тогда
				...
				Чтен.ЗаписатьИмяСвойства("withdrawal_type_other");
				Чтен.ЗаписатьЗначение(xmlString(Объект.ПричинаВыводаИзОборота_Строка));
...
Функция НаСервереФормируемвыводизОборотТабак(Thumbprint_1,session_ui,Boundary)
...
//Для Каждого Стр Из Объект.Товары Цикл
		...
		Чтен.ЗаписатьЗначение(xmlString(Объект.ПричинаВыводаИзОборота_Табак));
	...
	//Возврат Текст;
			...
			ВремФ=ПолучитьИмяВременногоФайла("json");
				...
				текст_1=Новый ТекстовыйДокумент;
				текст_1.ДобавитьСтроку(Текст);
				текст_1.Записать(ИмяВр,КодировкаТекста.UTF8);
			   ...
			   ФайлОтправкиНачало.ЗаписатьСтроку("Content-Disposition: form-data; name=""jsonFile""; filename=""" +"json.json"+ """"); 				
				...
				Возврат Новый Структура("Текст,Дв,РазмерФайла",Текст,ДвоичнД,РазмерФайла)
	
	
КонецФункции

Функция СформируемТелоЗапросаДляТабака(Текст,Подписанный)
		...
		Чтен.ЗаписатьЗначение("WRITE_OFF");
		...
		Чтен.ЗаписатьКонецОбъекта();
		Текст_2=Чтен.Закрыть();
		Текст_2=СтрЗаменить(Текст_2,Символы.ПС,"");
		Текст_2=СтрЗаменить(Текст_2,Символы.ВК,"");
	  Возврат Текст_2
	//"{
	//	|""product_document"":"""+СформироватьФайлДляОтправки_1.Текст+""",
	//	|""document_format"": ""MANUAL"",";
	//	
	//		Тело=Тело+"
	//		|""type"":""WRITE_OFF"",";
...
&НаКлиенте
Процедура ОтправитьВыводИзОборотаТабак(УжеОтправили=Ложь,Thumbprint_1)
	...
	Если не ЗначениеЗаполнено(Срегистра) тогда
		...
		Boundary="";
	СформироватьФайлДляОтправки_1=НаСервереФормируемвыводизОборотТабак(Thumbprint_1,"",Boundary);
	...
	Тело=СформироватьФайлДляОтправки_1.Текст;
		...
		//СформироватьФайлДляОтправки,КодировкаТекста.UTF8,Ложь));
		//Добав=base64Строка(ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ПолучитьДвоичныеДанныеИзСтроки_Сервер(
		//СформироватьФайлДляОтправки_1.Текст,КодировкаТекста.UTF8,Ложь));
		
		//Добав=СтрЗаменить(Добав,Символы.ПС,"\r\n");
		//Добав=СтрЗаменить(Добав,Символы.ВК,"\r\n");
		//Добав=СтрЗаменить(Добав,"\r\n","");

//		ДляПодписания=Base64Строка(СформироватьФайлДляОтправки_1.Дв);
		ДляПодписания=СформироватьФайлДляОтправки_1.Текст;
		ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,ДляПодписания,Thumbprint_1,Подписанный,Истина,Ложь,,,,);
	...
	Иначе
		...
		//Thumbprint_1.АдресКАПИ="ismotp.crptech.ru";
				Если ЗначениеЗАполнено(Thumbprint_1.АдресПрокси) Тогда
					Прокси=Новый ИнтернетПрокси;
					Прокси.Установить("http",Thumbprint_1.АдресПрокси,Thumbprint_1.ПортПрокси,Thumbprint_1.ПользовательПрокси,Thumbprint_1.ПарольПрокси);
...
//	Тело=СформируемТелоЗапросаДляТабака(СформироватьФайлДляОтправки_1.Текст,Подписанный);
	...
	//		
			...
			HTTPЗапрос=Новый HTTPЗапрос("api/v3/true-api/documents/dropped-out/create");
			//	HTTPЗапрос=Новый HTTPЗапрос("api/v3/true-api/lk/documents/create?pg="+Thumbprint_1.Что);
		//HTTPЗапрос=Новый HTTPЗапрос("documents/dropped-out/create");
	//	HTTPЗапрос.Заголовки.Вставить("Content-Type","application/json;charset=UTF-8");
		HTTPЗапрос.Заголовки.Вставить("Authorization","Bearer "+СРегистра);
		HTTPЗапрос.Заголовки.Вставить("X-Signature",Подписанный);
	...
	//	HTTPЗапрос.Заголовки.Вставить("Content-Length",Формат(стрДлина(Тело),"ЧГ="));
		HTTPЗапрос.Заголовки.Вставить("Content-Length",Формат(СформироватьФайлДляОтправки_1.РазмерФайла,"ЧГ="));
			//	HTTPЗапрос.Заголовки.Вставить("accept","application/json");
		
		//HTTPЗапрос.Заголовки.Вставить("Host","ismotp.crptech.ru");
		///Константин           
		//Имя=ПолучитьИмяВременногОФайла("TXT");
		//СформироватьФайлДляОтправки_1.Дв.Записать(Имя);
		HTTPЗапрос.УстановитьТелоИзДвоичныхДанных(СформироватьФайлДляОтправки_1.Дв);
		...
		//	#КонецЕсли
		//		HTTPЗапрос.УстановитьТелоИзСтроки(Тело,КодировкаТекста.ANSI);
		//HTTPЗапрос.УстановитьИмяФайлаТела(Имя);
		Ответ=HTTPСервисЗапрос.ОтправитьДляОбработки(HTTPЗапрос);
		Текст1=Ответ.ПолучитьТелоКакСтроку();
	...
	ИначеЕсли Ответ.КодСостояния=401 		ИЛИ Ответ.КодСостояния=502    Тогда
			...
			Иначе
				ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.Получение_Токена(Thumbprint_1);
				ОтправитьВыводИзОборотаТабак(Истина,Thumbprint_1)			
			КонецЕсли
	Иначе
		//	Если УжеОтправили=Истина Тогда
				Объект.Статус=ПредопределенноеЗначение("Перечисление.СтатусыДокументов_ХамелеонЦРПТ.Отказ");
...
Процедура ОтправитьЗаявку_1(Команда)
	//++ МВ:Овчинников 25.06.2021  •0
	Если Объект.Товары.Получить(0).ДатаПроизводства > ТекущаяДата() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка отправки в ЦРПТ. Документ %1 имеет дату производства больше текущей'");
		Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%1", Объект.Ссылка);
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;		//-- МВ:Овчинников 25.06.2021
...
Процедура ОтправитьЗаявку(Команда,УжеОтправили=Ложь,Черновик=Ложь) Экспорт
	...
	Если Объект.Товары.Получить(0).ДатаПроизводства > ТекущаяДата() Тогда
		...
		Возврат;	
	КонецЕсли;	
	//++ МВ:Овчинников 08.07.2021  •0
	Если НЕ ЗначениеЗаполнено(Объект.Товары.Получить(0).НомерВСД)  Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Ошибка отправки в ЦРПТ. В документе %1 не заполнен НомерВСД'");
		Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%1", Объект.Ссылка);
		Сообщение.Сообщить();
		Возврат;	
	КонецЕсли;	
	//-- МВ:Овчинников 08.07.2021
...
//	Возврат;
	...
	КонецЕслИ;
	Если Thumbprint_1.НаименованиеСтенда="Молоко" 
		И ЗапретОтправкиОтчетаОНанесенииБудущейДатой=Истина
		Тогда
		
		Если НачалоДня(Объект.ДатаПроизводства)>НачалоДня(ТекущаяДата()) тогда
			Сообщить("Ввод в оборот "+Строка(объект.Ссылка)+" нелья отправлять. Дата производства больше текущей даты");
			Возврат;
		Конецесли;
	Конецесли;
	...
	Если Объект.ТипДокумента=ПредопределенноеЗначение("Перечисление.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ВыводИзОборота")
		...
		Если 	Thumbprint_1.НаименованиеСтенда="Табак"
			...
			ОтправитьВыводИзОборотаТабак(,Thumbprint_1);
			Возврат;
		КонецЕсли;
	...
	Если НеПроверятьБалансПриОправкеОтчетаОНанесении=Ложь Тогда
		...
		Если Объект.ТипДокумента=ПредопределенноеЗначение("Перечисление.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ОтчетОбИспользовании") ТОгда
			...
			Иначе
					...
					Если Ответ.КодСостояния<>200 Тогда
						Ток=ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.Получение_Токена(Thumbprint_1);	
					Иначе
						NNN_Баланс=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗаполнитьСтруктуруИзОтветаJSON(Ответ.ПолучитьТелоКакСтроку());
						
						БалансПолучили=Истина;
					КонецЕсли;
						
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Ток) Тогда
				Если БалансПолучили=Ложь Тогда
					Если ЗначениеЗАполнено(Thumbprint_1.АдресПрокси) Тогда
							Прокси=Новый ИнтернетПрокси;
							Прокси.Установить("http",Thumbprint_1.АдресПрокси,Thumbprint_1.ПортПрокси,Thumbprint_1.ПользовательПрокси,Thumbprint_1.ПарольПрокси);
			...
			Конецесли;
			Попытка
				Если БалансПолучили=Истина Тогда
					Для Каждого Стр Из NNN_Баланс Цикл 
						
						ТипГруппы=Стр.productGroupId;
						Если ТипГруппы=1 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Одежда" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Тип="Предметы одежды, бельё постельное, столовое, туалетное и кухонное";
						ИначеЕсли ТипГруппы=2 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Обувь" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Обувные товары";
						ИначеЕсли ТипГруппы=3 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Табак" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Табачная продукция";
						ИначеЕсли ТипГруппы=4 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Парфюм" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Духи и туалетная вода";
						ИначеЕсли ТипГруппы=5 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Шины" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Шины и покрышки пневматические резиновые новые";
						ИначеЕсли ТипГруппы=6 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Фото" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Фотокамеры (кроме кинокамер), фотовспышки и лампы-вспышки";
						ИначеЕсли ТипГруппы=8 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Молоко" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Молочная продукция";
						ИначеЕсли ТипГруппы=9 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Велосипеды" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Велосипеды и велосипедные рамы";
						ИначеЕсли ТипГруппы=10 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Кресла-коляски" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Кресла-коляски";
						ИначеЕсли ТипГруппы=12 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Альтернативный табак" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Альтернативная табачная продукция";
						ИначеЕсли ТипГруппы=13 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Питьевая вода" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Упакованная вода";
							
						ИначеЕсли ТипГруппы=14 Тогда
							//Если Thumbprint_1.НаименованиеСтенда="Питьевая вода" Тогда
							//	Баланс=Стр.balance;
							//	Прервать;
							//Конецесли;
							//Добав.Тип="Товары из натурального меха";
						ИначеЕсли ТипГруппы=15 Тогда
							Если Thumbprint_1.НаименованиеСтенда="Пиво" Тогда
								Баланс=Стр.balance;
								Прервать;
							Конецесли;
							//Добав.Тип="Пиво, напитки, изготавливаемые на основе пива, слабоалкогольные напитки";
						ИначеЕсли ТипГруппы=16 Тогда
							//Если Thumbprint_1.НаименованиеСтенда="Пиво" Тогда
							//	Баланс=Стр.balance;
							//	Прервать;
							//Конецесли;
							//Добав.Тип="Никотиносодержащая продукция";
						ИначеЕсли ТипГруппы=17 Тогда
							//Если Thumbprint_1.НаименованиеСтенда="Пиво" Тогда
							//	Баланс=Стр.balance;
							//	Прервать;
							//Конецесли;
							//Добав.Тип="Биологические активные добавки к пище";
						Конецесли;
					КонецЦикла
				Конецесли;
			Исключение
			КонецПопытки;
			Если Баланс<>-99999		Тогда
				
				Сумма=Объект.СерийныеНомера.Количество()*0.6;
				Если Сумма>Баланс Тогда
					
					Сообщить("Баланс "+Формат(Баланс,"ЧГ=")+", на счету не хватает средств для отправки отчета о нанесении. Необходимо "+Формат(Сумма,"ЧГ="));
					Возврат;
				Конецесли;
	...
	//++ МВ:Овчинников 25.06.2021  •0
	 СформироватьВводВОборотДляАрлы();
	//-- МВ:Овчинников 25.06.2021	
КонецПроцедуры
...
//	Чтен.ЗаписатьКонецОбъекта();
	...
	Для Каждого Стр Из Объект.СерийныеНомера Цикл
		...
		Если ЗначениеЗаполнено(НСтр.НомерВСД) И УказыватьНомерВСД=Истина Тогда
			Чтен.ЗаписатьИмяСвойства("vsd_number");
					ГУИД=НСтр.НомерВСД;
			Если СтрДлина(ГУИД)>36 тогда
		//		
...
Процедура СохранитьВTXT(Команда)
	...
	//	Для Каждого Стр_1 Из Форм Цикл
			...
			Номер="";
			ДвоичнД=СохранитьФайлНаДИске_ЦРПТ(Объект.Ссылка,Номер);
...
Функция СохранитьФайлНаДИске_ЦРПТ(Ссылка,Номер)
...
Исключение
	...
	ПоказатьВводЧисла(Новый ОписаниеОповещения("ПолсеВыбора",ЭтаФорма),99999,"Укажите количество серийный номеров на которое необходимо разбить документ",10,0);
...
Процедура ОПределитьУпаковки(Thumbprint_1)
	...
	Молоко=Thumbprint_1.НаименованиеСтенда="Молоко";
		...
		Для Каждого Стр_2 Из Объект.СерийныеНомера Цикл
			Если Не ЗначениеЗАполненО(Стр_2.ТипУпаковки) Тогда
				...
				Если КоличествоБольше400 Тогда
					...
					Иначе
						Если Молоко Тогда
							Если СтрДлина(Стр_2.cis)=24 Тогда
									ТипУпаковки=Ложь;
								Иначе
									ТипУпаковки=Истина;
...
Процедура ПоказатьДетальнуюИнформацию(Команда)
	...
	Сайт=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ПолучитьСайт(Истина);
...
Процедура 	ДобавитьПродукцию_ХамелеонЦРПТ()
	...
	Для Каждого Стр_1 Из ОБъект.НомераВСД Цикл
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				// Вставить обработку выборки ВыборкаДетальныеЗаписи
				Если МассивПродукций.НайтиПоЗначению(ВыборкаДетальныеЗаписи.Ссылка)<>Неопределено Тогда
...
&НаСервере
Процедура ПерезаполнитьПоОтчетуОНанесенииНаСервере()
	// Вставить содержимое обработчика.
	Если ЗначениеЗаполнено(Объект.ДокОснование)
		И ТипЗнч(Объект.ДокОснование)=Тип("ДокументСсылка.ВводВОборот_ХамелеонЦРПТ") 
		И Объект.ДокОснование.ТипДокумента=Перечисления.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ОтчетОбИспользовании
		И
(		Объект.Статус=Перечисления.СтатусыДокументов_ХамелеонЦРПТ.ПодготовленКОтправке
		ИЛИ Не ЗначениеЗаполнено(Объект.Статус) ИЛИ ОБъект.Статус=Перечисления.СтатусыДокументов_ХамелеонЦРПТ.Отказ)
		Тогда
		Документоснование=Объект.ДокОснование;
	ЗаполнитьЗначенияСвойств(Объект,Объект.ДокОснование,,"Номер,Дата,_Order_ID,Статус,ДокОснование,Товары,СерийныеНомера,GTINДляПубликации,НомераВСД");
	
	Объект.ТипДокумента=Перечисления.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ВводВОборот;
			            Объект.Товары.Очистить();
			Для Каждого Стр_1_Д Из Документоснование.Товары Цикл
				Стр_1=Объект.Товары.Добавить();
				...
				Если ЗначениеЗаполнено(Стр_1.ДатаПроизводства) Тогда
					 Объект.ДатаПроизводства=Стр_1.ДатаПроизводства
			...
			Для Каждого Стр_1_Д Из Документоснование.СерийныеНомера Цикл
				Стр_1=Объект.СерийныеНомера.Добавить();
				ЗаполнитьЗначенияСвойств(Стр_1,Стр_1_Д,,"Статус,Отправлена");
				
			cis=Стр_1.cis;
			Если (Объект.ТекущийПользователь.СайтыВходаВСистему.Наименование="Молоко" И Найти(cis,"%1D")>=25)
			ИЛИ (Объект.ТекущийПользователь.СайтыВходаВСистему.Наименование<>"Молоко" И Найти(cis,"%1D")>=32)
			тогда		
			//Если Найти(cis,"%1D")>0  Тогда
				cis=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ПолучитьКороткийНомерКМ_Сайт(
				cis,Объект.ТекущийПользователь.СайтыВходаВСистему.Наименование)
			Иначе
...
&НаКлиенте
Процедура ПерезаполнитьПоОтчетуОНанесении(Команда)
	ПерезаполнитьПоОтчетуОНанесенииНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура Таблица1НомерВСДПриИзменении(Элемент)
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ПоказатьДетИнфуИзТоваров(Команда)
	// Вставить содержимое обработчика.
	...
	НачатьЗапускПриложения(Новый ОписаниеОповещения("ПослеР",ЭтаФорма),Сайт+"/pub/operatorui?_action=findVetDocumentFormByUuid&uuid="+Элементы.Товары.ТекущиеДанные.НомерВСД);
	Конецесли;
	
КонецПроцедуры

&НаКлиенте
Процедура СверкаКМ(Команда)
	// Вставить содержимое обработчика.
	...
	Для Каждого Стр Из Объект.СерийныеНомера Цикл
		...
		cis=Стр.cis;
		Если Найти(cis,"%1D")>=25 Тогда
			cis=РасКодСтр_Заменой(cis); 
			//РаскодироватьСтроку(cis,СпособКодированияСтроки.КодировкаURL);
			
		Конецесли;
		Добав=МассивДляПроверки.Добавить();
		Добав.КМ=cis;
		Добав.Строка=Стр.ПолучитьИдентификатор();
	...
	Удаляем_1=Новый Массив;
	Добавляем_2=Новый Массив;
//	Удаляем_2=Новый Массив;
	...
	Для Каждого Стр_1 Из МассивКМ Цикл
		...
		НСтр=МассивДляПроверки.НайтиСтроки(Новый Структура("КМ",cis));
		Нашли=ЛожЬ;
		Для Каждого Стр_2 Из НСтр Цикл
			Если Стр_2.КМ=cis Тогда
		...
		Если Нашли=Истина Тогда
			//Удаляем_2.Добавить(Стр_1.Строка);
			Удаляем_1.Добавить(Стр_2);
		Иначе
			Добавляем_2.Добавить(cis);
			Сообщить("В документе "+Строка(Объект.Ссылка)+" нет КМ "+cis);
		Конецесли;
		
	Конеццикла;
	  
	Для Каждого Стр Из Удаляем_1 Цикл
		
		МассивДляПроверки.Удалить(Стр);
		
	Конеццикла;
	Если МассивДляПроверки.Количество()>0 Тогда
		Для Каждого Стр Из МассивДляПроверки Цикл
			Сообщить("В загруженном файле нет КМ "+Стр.КМ);
		Конеццикла;
		
	КонецеслИ;
	
	Если МассивДляПроверки.Количество()>0  
//		Добавляем_2.Количество()>0 
Тогда
		ТекстВопроса="";
		Если МассивДляПроверки.Количество()>0 Тогда
			ТекстВопроса="Удалить КМ, которых нет в загруженном файле?"
		Конецесли;
		//Если Добавляем_2.Количество()>0 Тогда
		//	Если ЗначениеЗаполнено(ТекстВопрсоа) Тогда
		//		ТекстВопрсоа=ТекстВопрсоа+Символы.ПС;
		//	КонецЕсли;
		//	ТекстВопрсоа=ТекстВопрсоа+"Добавить КМ, загруженные";
		//Конецесли;
		
		ПоказатьВопрос(Новый Описаниеоповещения("ПослеВопросаОДобавленииУдалении",ЭтаФорма,МассивДляПроверки),
...
&НаКлиенте
Процедура ПослеВопросаОДобавленииУдалении(РР,ДД) Экспорт
	Если РР=КодВозвратаДиалога.Да Тогда
		
		Для каждого Стр Из ДД Цикл
			НСтр=Объект.СерийныеНомера.НайтиПоИдентификатору(Стр.Строка);
			Объект.СерийныеНомера.Удалить(НСтр);
			СтрокаТовары=Объект.Товары.НайтиСтроки(Новый Структура("НомерСтрокиСвязи",
			НСтр.НомерСтрокиСвязи));
			СтрокаТовары[0].КоличествоКМ=СтрокаТовары[0].КоличествоКМ-1;
			
			СтрокаТовары[0].КоличествоНомеровКМ=Макс(0,СтрокаТовары[0].КоличествоНомеровКМ-1);
...
КонецПроцедуры
&НаСервереБезКонтекста
Функция РасКодСтр_Заменой(Строка) Экспорт
	cis=РаскодироватьСтроку(Строка,СпособКодированияСтроки.КодировкаURL);
	//cis=Сред(cis,1,Найти(cis,Символ(29))-1);
	cis=СтрЗаменить(cis,Символ(29),"");
	Возврат cis
...
&НаКлиенте
Процедура ПроверитьСтатусыКМИОбработатьДокументОтчетОНанесении(Команда)
	// Вставить содержимое обработчика.
	Если ОБъект.ТипДокумента=ПредопределенноеЗначение("Перечисление.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ОтчетОбИспользовании") Тогда
		ПолучитьСтатусыКМВсе(Команда);
		Для Каждого Стр Из ОБъект.СерийныеНомера Цикл
			Если Стр.Статус<>"INTRODUCED"
				И Стр.Статус<>"APPLIED"  Тогда
				
				Сообщить("КМ "+Стр.cis+" в статусе "+Стр.Статус);
				Возврат;
...
&НаСервере
Процедура ЛогСтатусЗапросНаСервере()
	// Вставить содержимое обработчика.
	Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.Ссылка,"Запрос на статус отчета в СУЗ")).Файл;
	Если Хр<>Неопределено тогда
		Отправка=Хр.Получить();
	Конецесли;
КонецПроцедуры
...
&НаКлиенте
Процедура ЛогСтатусЗапрос(Команда)
	ЛогСтатусЗапросНаСервере();
КонецПроцедуры


&НаСервере
Процедура ЛогСтатусОтветНаСервере()
	// Вставить содержимое обработчика.
	Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.Ссылка,"Ответ на запрос на статус отчета в СУЗ")).Файл;
	Если Хр<>Неопределено тогда
		ОтветОтЦРПТ=Хр.Получить();
	Конецесли;
КонецПроцедуры
...
&НаКлиенте
Процедура ЛогСтатусОтвет(Команда)
	ЛогСтатусОтветНаСервере();
КонецПроцедуры


//++ МВ:Овчинников 29.06.2021  •0
&НаКлиенте
Процедура ОтправитьОтчетОбИспользованииАрле(Thumbprint_1)
	Сообщение = Новый СообщениеПользователю;
	
	Если НЕ Thumbprint_1.НаименованиеСтенда = "Молоко" Тогда
		Возврат;
	КонецЕсли;
	СтруктураНастроек = ПолучитьНастройкиАрлы();
	Если СтруктураНастроек = Неопределено Тогда
		Возврат;
	КонецЕсли;	
	
	Адрес = СтруктураНастроек.Адрес;
	АдресHTTPЗапрос = СтруктураНастроек.АдресHTTPЗапрос;
	Login = СтруктураНастроек.Логин;
	Password = СтруктураНастроек.Пароль;

	МассивФайлов=СформироватьОтчетОбИспользованииМассивДляАрлы(Thumbprint_1);
	Сч=0;
		//Адрес = "api.dev.arla.kvinta.io"; https://api.prod.arla.kvinta.io/arla-gateway/v1/captureOmsUtilisation
		//Login = "capture-molvest";
		//Password = "aw6QDEam";
		...
		Для Каждого СформироватьФайлДляОтправки_3 ИЗ МассивФайлов Цикл
			...
			Если Не ЗначениеЗаполнено(Адрес) ИЛИ Не ЗначениеЗаполнено(АдресHTTPЗапрос) Тогда
				
				ПоказатьПредупреждение(,"Не не заполнен адрес отправки Арле");
				Возврат;
				
			КонецЕсли;
			
				
				HTTPСервисЗапрос=Новый HTTPСоединение(Адрес,,Login,Password,,300,Новый ЗащищенноеСоединениеOpenSSL());//HTTPСоединение(Адрес,Порт,,,,Thumbprint_1.Таймаут,Защищ);   //TODO
				Подписанный="";
				HTTPЗапрос=Новый HTTPЗапрос(АдресHTTPЗапрос);//("arla-gateway/v1/captureOmsUtilisation"); //("/arla-extensions/v1/captureOmsUtilisation");  
				HTTPЗапрос.Заголовки.Вставить("Content-Type","application/json");//("Content-Type","application/json;charset=UTF-8");
				HTTPЗапрос.УстановитьТелоИзСтроки(Тело);
				Ответ=HTTPСервисЗапрос.ОтправитьДляОбработки(HTTPЗапрос);
				Текст1=Ответ.ПолучитьТелоКакСтроку();				
				...
				Если Ответ.КодСостояния=202 ИЛИ Ответ.КодСостояния=200 Тогда
					...
					//НСтр.Статус=ПредопределенноеЗначение("Перечисление.СтатусыДокументов_ХамелеонЦРПТ.ОтправленЧерновик");
					//НСтр.order_ID=NNN.reportId;
					Сообщить(NNN.status+" "+" файл отправлен Арле: "+ Объект.Ссылка);
					
				
					Заголовок = НСтр("ru = '(МВ).Выгрузка Арле Ввод в оборот.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
					ИнформационныйТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отправка отчета Арле. Статус ""%1"" из документа:
					|%2'"),
					NNN.status,
					Объект.Ссылка);
					...
					//Объект.Статус=ПредопределенноеЗначение("Перечисление.СтатусыДокументов_ХамелеонЦРПТ.Отправлен");
					Объект.МВ_ОтчетАрлеОтправлен = Истина;
					ЭтаФорма.Записать();
					...
					Попытка
						...
						//Исключение
							Попытка
								//НСтр.ОшибкаОтправки=NNN.error_description;
								Если NNN.error="access_denied" Тогда
									Сообщить(NNN.error_description+" "+" сайт не доступен. Повторите отправку позже");
								Иначе
									Сообщить(NNN.globalErrors[0].error);
								КонецЕсли;
								Заголвок = НСтр("ru = '(МВ).Ошибка выгрузки Арле ввод в оборот.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
								ИнформационныйТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Ошибка отправки отчета Арле. Статус ""%1"" из документа:
								|%2'"),
								NNN.error,
								Объект.Ссылка);
								
								ДобавитьЗаписьЖурналаРегистрации(Заголвок,ИнформационныйТекст);	
							Исключение
								Сообщить(Текст1);
							КонецПопытки;
...
&НаКлиенте
Процедура КомандаВыбратьВСД(Команда, ЗаписатьИзменения = Ложь) Экспорт
	...
	ТекПользователь = Объект.ТекущийПользователь;
	ОрганизацияМеркурия = ПолучитиьОрганизациюМеркурияПоХамелеону(Объект.Организация);
	GTIN = ПолучитьGTIN(Объект.Товары.Получить(0).GTIN);
	ДатаПроизводства =  Объект.Товары.Получить(0).ДатаПроизводства;
	...
	НомерПартии = СокрЛП(Объект.НомерПартии);
...
&НаКлиенте
Процедура КомандаСформироватьВводВОборотДляАрлы()
	
	Если Объект.МВ_ОтчетАрлеОтправлен Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Отчет отправлен Арле, для повторной отправки требуется убрать флаг об отправке'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
//++ МВ:Овчинников 29.06.2021  •0
&НаКлиенте
Процедура СформироватьВводВОборотДляАрлы()
	...
	Если НЕ Объект.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ВВодВОборот") ТОгда
	   // Сообщить("Для отгрузке Арле нужен ввод в оборот");	
		Возврат;
	Конецесли;
	
	Если НЕ ЗначениеЗаполнено(СокрЛП(Объект.НомерПартии)) ИЛИ СтрДлина(СокрЛП(Объект.НомерПартии)) < 10 ТОгда
	   // Сообщить("Это не Арла");	
		Возврат;
	Конецесли;

	ОтправитьОтчетОбИспользованииАрле(Thumbprint_1);
...
Процедура ВыборВСДЗавершение(ВСД, ДополнительныеПараметры) Экспорт
	...
	ВыборВСДЗавершениеНаСервере(ВСД,ДополнительныеПараметры);
...
//++ МВ:Овчинников 22.06.2021  •0
&НаСервере
Процедура ВыборВСДЗавершениеНаСервере(ВСД,ДополнительныеПараметры)
	...
	Иначе
		...
		Сообщение.Текст = СтрЗаменить(Сообщение.Текст, "%1", ДокументОбъект);
		Сообщение.Сообщить();
	КонецЕсли;
...
//++ МВ:Овчинников 24.06.2021  •0
&НаСервереБезКонтекста
Функция ПолучитьНастройкиАрлы()
	
	ТекДата = ТекущаяДатаСеанса();	
	МассивНастроек = Новый Массив;
	ПереченьНастроек = ПланыВидовХарактеристик.МВ_ПереченьДополнительныхНастроек;
	МассивНастроек.Добавить(ПереченьНастроек.АрлаВебАдресДляОтправкиОтчетовВводаВОборот);
	МассивНастроек.Добавить(ПереченьНастроек.АрлаАдресHTTPЗапрос);
	МассивНастроек.Добавить(ПереченьНастроек.АрлаЛогин);
	МассивНастроек.Добавить(ПереченьНастроек.АрлаПароль);
	...
	Интерактивно = Истина;
	Адрес = СоответствиеНастроек.Получить(МассивНастроек[0]);
	АдресHTTPЗапрос = СоответствиеНастроек.Получить(МассивНастроек[1]);
	Если Адрес = Неопределено ИЛИ АдресHTTPЗапрос = Неопределено Тогда		
		Текст = НСтр("ru = 'Не смогли определить адрес выгрузки в Арлу, заполните значение для %1 .'");
		Если Интерактивно Тогда			
			...
			Сообщение.Сообщить();
		Иначе		
			ДобавитьЗаписьЖурналаРегистрации(НСтр("ru = '(МВ).Выгрузка Арле Ввод в оборот.'", ОбщегоНазначения.КодОсновногоЯзыка()), Текст,Истина);
	...
	СтруктураНастроек.Вставить("Адрес",Адрес);
	СтруктураНастроек.Вставить("АдресHTTPЗапрос",АдресHTTPЗапрос);
	СтруктураНастроек.Вставить("Логин",Логин);
	СтруктураНастроек.Вставить("Пароль",Пароль);

	Возврат СтруктураНастроек;

КонецФункции //ПолучитьНастройкиАрлы  //-- МВ:Овчинников 24.06.2021
...
//++ МВ:Овчинников 23.04.2021  •0
&НаСервере
Функция ТоварыНомерВСДПривидениекФорматуМеркурия(GUID_Меркурий)
	ГУИД = GUID_Меркурий;
	Если СтрЧислоВхождений(ГУИД, "-") < 8 Тогда 	
		Индекc = 5;
		ДобавленоПрефиксов = 0;
		Пока Индекc < СтрДлина(ГУИД) - 1 Цикл
			Если Сред(ГУИД, Индекc, 1) <> ("-") Тогда
				КоличествоСимволов = СтрДлина(ГУИД);
				ГУИД = ?(Индекc = 1, "", Сред(ГУИД, 1, Индекc-5 )) + Сред(ГУИД, Индекc-4, 4) + "-" + Сред(ГУИД, Индекc, КоличествоСимволов);
				Индекc = Индекc + 5;
				ДобавленоПрефиксов = ДобавленоПрефиксов + 1;
			Иначе
				Индекc = Индекc + 5;
			КонецЕсли; 		
		КонецЦикла;					
...
КонецФункции //-- МВ:Овчинников 23.04.2021