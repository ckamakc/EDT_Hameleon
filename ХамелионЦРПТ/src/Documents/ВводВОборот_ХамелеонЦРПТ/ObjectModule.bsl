	...
	Процедура ПриКопировании(ОбъектКопирования)  Экспорт //++ МВ:Овчинников 11.06.2021  •0
		...
		ЭтотОбъект.ВводВОборотПослеВводаВОтчет=Неопределено;
		ЭтотОбъект.ДокОснование=Неопределено;
		
		Если ТипЗнч(ОбъектКопирования.Ссылка)=Тип("ДокументСсылка.ВводВОборот_ХамелеонЦРПТ") Тогда
			ЭтотОБъект.ДокОснование=ОбъектКопирования.Ссылка;
		Конецесли;
		...
		Если ОбъектКопирования.ТипДокумента=Перечисления.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ОтчетОбИспользовании Тогда
			...
			Для Каждого Стр_1 Из СерийныеНомера Цикл
				cis=Стр_1.cis;
			Если (ТекущийПользователь.СайтыВходаВСистему.Наименование="Молоко" И Найти(Стр_1.cis,"%1D")>=25)
			ИЛИ (ТекущийПользователь.СайтыВходаВСистему.Наименование<>"Молоко" И Найти(Стр_1.cis,"%1D")>=32)
			//тогда		
		...
		Если ДанныеЗаполнения <> Неопределено Тогда
			...
			Если ДокументОснование <> Неопределено Тогда
				...
				Если Не ЗначениеЗаполнено(НомерПервичногоДокумента) Тогда
					НомерПервичногоДокумента=ДокОснование.Номер;
					
				Конецесли;
				ВидПервичногоДокумента=Перечисления.ТипПервичногоДокумента_ХамелеонЦРПТ.CONSIGNMENT_NOTE;
				ДатаПервичногоДокумента=ДокОснование.дата;
						...
						Если РезультатЗапроса_Д .Количество()>0 Тогда
								...
								Если РезультатЗапроса.Количество()>0 Тогда 
									...
									Пока КоличествоНеобходмо>0 Цикл
										Если КоличествоНеобходмо<=30000 Тогда
										...
										Иначе
											НадоСпсать=Мин(КоличествоНеобходмо,30000);
											...
											Для Сч=Смещение По Смещение+НадоСпсать-1 Цикл
												...
												ППП=РезультатЗапроса_Д.НайтиСтроки(Новый Структура("СерийныйНомер",Врег(cis)));
												Если ППП.Количество()=0 
													//И ППП.КоличествоОстаток<=0 
													Тогда
													Продолжить;
											...
											//	часыДо=Ложь;
												...
												Добав=ДокумС.Товары.НайтиСтроки(Новый Структура("GTIN,Годендо,ЧасыДо,ДатаПроизводства,НомерВСД",
												ДД.НаименованиеТовара,Годендо,ЧасыДо,ДатаПроизводства,НомерВСД));
												Если Добав.Количество()=0 Тогда
			...
			//								cis=Сред(cis,1,31);
					...
					ИначеЕсли ТипЗнч(ДокументОснование)=Тип("ДокументСсылка.ВводВОборот_ХамелеонЦРПТ") Тогда
						...
						Для Каждого Стр_1111 Из РезультатЗапроса_М Цикл
							Стр_1111.СерийныйНомер=Врег(Стр_1111.СерийныйНомер);
						Конеццикла;
				...
				Если Не ЗначениеЗАполнено(стр_1.ДекларацияИлиСертификатСоответствия) тогда
										стр_1.ДекларацияИлиСертификатСоответствия=РЕгистрыСведений.ПодтверждающиеДокументы_ХамелеонЦРПТ.ПолучитьПоследнее(
												...
												Конецесли;
												Если Не ЗначениеЗАполнено(стр_1.КодТНВЭД) Тогда
			...
			Конеццикла;
						...
						Если (ТекущийПользователь.СайтыВходаВСистему.Наименование="Молоко" И Найти(cis,"%1D")>=25)
						ИЛИ (ТекущийПользователь.СайтыВходаВСистему.Наименование<>"Молоко" И Найти(cis,"%1D")>=32)
						тогда		
						...
						ИначеЕсли ТекущийПользователь.СайтыВходаВСистему.Наименование="Альтернативный табак"
							...
							Конецесли;
									ППП=РезультатЗапроса_М.НайтиСтроки(Новый Структура("СерийныйНомер",Врег(cis)));
									Если ППП=Неопределено ИЛИ (ППП.Количество()=0 
											//И ППП.КоличествоОстаток<=0
											) 
											Тогда
	...
	Процедура ОбработкаПроведения(Отказ, РежимПроведения)
		...
		ЗаказыКМ=Новый ТаблицаЗначений;
		ЗаказыКМ.Колонки.Добавить("Заказ");
		ЗаказыКМ.Колонки.Добавить("Ссылка");
		//ЗаказыКМ.Колонки.Добавить("Заказ");
		...
		Если ЭтоТОбъект.Статус=перечисления.СтатусыДокументов_ХамелеонЦРПТ.Обработан Тогда
			   ...
			   МаркиКМ=Новый списокЗначений;
			   МаркиКМ_Табица=Новый ТаблицаЗначений;
			МаркиКМ_Табица.Колонки.Добавить("КМ");
			МаркиКМ_Табица.Индексы.Добавить("КМ");
		//	Марки=СерийныеНомера.ВыгрузитьКолонку("cis");
			...
			Иначе
				...
				Если            Найти(Стр.cis,"%1D")>=25 Тогда
			...
			КонецЕсли;
				...
				МаркиКМ.Добавить(Стр.cis);	
				Добав=МаркиКМ_Табица.Добавить();
				Добав.КМ=Стр.cis;
			...
			КонецЦикла;
				...
				Если Настройки.СоответствоватьТребованиямЦРПТ Тогда
				...
				Запрос.Текст = 
					"ВЫБРАТЬ
					|	НомераКМ_ХамелеонЦРПТ.НомерКМ,
					|	НомераКМ_ХамелеонЦРПТ.ДокументЗаказа,
					|	НомераКМ_ХамелеонЦРПТ.НомерСтроки_1,
					|	НомераКМ_ХамелеонЦРПТ.OrderID,
					|	НомераКМ_ХамелеонЦРПТ.НомерКМ_Короткий,
					|	НомераКМ_ХамелеонЦРПТ.КМНапечатан
					|ИЗ
					|	РегистрСведений.НомераКМ_ХамелеонЦРПТ КАК НомераКМ_ХамелеонЦРПТ
					|ГДЕ
					|	НомераКМ_ХамелеонЦРПТ.НомерКМ_Короткий В(&НомерКМ_Короткий)";
				
				Запрос.УстановитьПараметр("НомерКМ_Короткий", МаркиКМ);
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					НСтр=МаркиКМ_Табица.НайтиСтроки(Новый Структура("КМ",ВыборкаДетальныеЗаписи.НомерКМ_Короткий));
					
					Для Каждого Ср_11 Из НСтр Цикл
						Если Ср_11.КМ=ВыборкаДетальныеЗаписи.НомерКМ Тогда
							
							
							
							РегДв_1=РегистрыСведений.НомераКМ_ХамелеонЦРПТ.СоздатьНаборЗаписей();
							РегДв_1.Отбор.НомерКМ.Установить(ВыборкаДетальныеЗаписи.НомерКМ);
							РегДв_1.Отбор.НомерСтроки_1.Установить(ВыборкаДетальныеЗаписи.НомерСтроки_1);
							РегДв_1.Отбор.ДокументЗаказа.Установить(ВыборкаДетальныеЗаписи.ДокументЗаказа);
							РегДв_1.Записать(Истина);
							
							Добав__=РегистрыСведений.НомераКМ_ХамелеонЦРПТ.СоздатьНаборЗаписей();
							Добав__.Отбор.НомерКМ.Установить(ВыборкаДетальныеЗаписи.НомерКМ_Короткий);
							Добав__.Отбор.ДокументЗаказа.Установить(ВыборкаДетальныеЗаписи.ДокументЗаказа);
							Добав__.Отбор.НомерСтроки_1.Установить(ВыборкаДетальныеЗаписи.НомерСтроки_1);
							Добав_Рег=Добав__.Добавить();
							ЗаполнитьЗначенияСвойств(Добав_Рег,ВыборкаДетальныеЗаписи);
							Добав_Рег.НомерКМ=Добав_Рег.НомерКМ_Короткий;
							Добав__.Записать(Истина);
							
							Если ЗаказыКМ.Найти(ВыборкаДетальныеЗаписи.ДокументЗаказа,"Ссылка")=Неопределено Тогда
								Добав_З=ЗаказыКМ.Добавить();
								Добав_З.Ссылка=ВыборкаДетальныеЗаписи.ДокументЗаказа;
								ПолОб=Добав_З.Ссылка.ПолучитьОбъект();
								Добав_З.Заказ=ПолОб;
				...
				Запрос.Текст = 
					"ВЫБРАТЬ
					|	ЗаказМарок_ХамелеонЦРПТКМИзAPI.Ссылка,
					|	ЗаказМарок_ХамелеонЦРПТКМИзAPI.НомерСтроки
					|ИЗ
					|	Документ.ЗаказМарок_ХамелеонЦРПТ.КМИзAPI КАК ЗаказМарок_ХамелеонЦРПТКМИзAPI
					|ГДЕ
					|	ЗаказМарок_ХамелеонЦРПТКМИзAPI.НомерКМ_Короткий В(&НомерКМ_Короткий)";
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					НСтр=МаркиКМ_Табица.НайтиСтроки(Новый Структура("КМ",ПолОб.КМИзAPI[ВыборкаДетальныеЗаписи.НомерСтроки-1].НомерКМ_Короткий));
					
					Для Каждого Ср_11 Из НСтр Цикл
						Если Ср_11.КМ=ПолОб.КМИзAPI[ВыборкаДетальныеЗаписи.НомерСтроки-1].НомерКМ_Короткий Тогда
					
								
								
								// Вставить обработку выборки ВыборкаДетальныеЗаписи
								Заказ__КМ=ЗаказыКМ.Найти(ВыборкаДетальныеЗаписи.Ссылка);
								Если Заказ__КМ=Неопределено Тогда
									ПолОб=ВыборкаДетальныеЗаписи.Ссылка.ПолучитьОбъект();
									Добав___=ЗаказыКМ.Добавить();
									Добав___.Заказ=ПолОб;
									Добав___.Ссылка=ВыборкаДетальныеЗаписи.Ссылка;
								Иначе
									ПолОб=Заказ__КМ.Заказ;
								Конецесли;
								
								
								ПолОб.КМИзAPI[ВыборкаДетальныеЗаписи.НомерСтроки-1].НомерКМ=ПолОб.КМИзAPI[ВыборкаДетальныеЗаписи.НомерСтроки-1].НомерКМ_Короткий;
								Прервать;
							Конецесли;
						Конеццикла;
				КонецЦикла;
				...
				Для Каждого Стр_2222 Из ЗаказыКМ Цикл
					Если Стр_2222.Заказ.Проведен Тогда
						Стр_2222.Заказ.Записать(режимЗаписиДокумента.Проведение)
					Иначе
						Стр_2222.Заказ.Записать(режимЗаписиДокумента.Запись)
					Конецесли;
	...
	//					cis=РаскодироватьСтроку(cis,СпособКодированияСтроки.КодировкаURL);
		...
		ИначеЕсли РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ДелатьДвиженияВЛюбомСлучаеПоОтчетуОНанесении
			И ЭтотОбъект.ТипДокумента=Перечисления.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ОтчетОбИспользовании Тогда
	...
	Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
		...
		Если не ЗначениеЗаполнено(Ссылка) тогда
			ЭтотОБъект.ПродолжитьВводВОборот=РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ДелатьВводВОборотПослеОтчетаОНанесении;
			//ТекущийПользователь.СоздаватьИОтправлятьВводВОборотПослеЭмитирования;
			...
			Если ЭтотОбъект.Статус=Перечисления.СтатусыДокументов_ХамелеонЦРПТ.Обработан Тогда
				...
				Для Каждого Стр Из ЭтотОбъект.СерийныеНомера Цикл
								...
								Если            Найти(Стр.cis,"%1D")>=25 Тогда
									...
									СерийныйНомер=Сред(cis,1,45);

								Стр.cis=СерийныйНомер;
