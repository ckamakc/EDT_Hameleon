		...
		Пока МассивКМ.Количество()>0 Цикл
			...
			Для Каждого Стр_1 Из ВернутьСтатусы Цикл
				...
				Если НСтрок.Количество()=0 Тогда
					...
					Для каждого Стр___20 Из Объект.СерийныеНомера Цикл
						Если найти(Стр___20.cis,Стр_1.cis)=1 Тогда
							...
							Стр___20.НаКомиссии=Стр_1.НаКомиссии;
				...
				Иначе
					...
					Нстрок[0].НаКомиссии=Стр_1.НаКомиссии;
	...
	Если Объект.ОтправлятьКМВместоАгрегаций Тогда
		...
		//УдаляемКоробки=Новый Массив;
			Для Каждого Стр Из Объект.СерийныеНомера Цикл
				...
				строказ="";
				если стр.ТипУпаковки=Перечисления.Тип_КМ_ХамелеонЦРПТ.Упаковка
					И НСтр.GTIN.ТипУпаковки.Наименование<>"BUNDLE"
					И НСтр.GTIN.ТипУпаковки.Наименование<>"SET"
					тогда 
					...
					Для Каждого Стр_111 Из СписокКМ Цикл
						...
						строказ=строказ+""""+стрзаменить(?(НСтр.цена=0,"",xmlstring(Окр(НСтр.цена/СписокКМ.Количество(),2))*Коэффициент),",",".")+""","""+
						стрзаменить(xmlstring(?(НСтр.Ндс=0,"",окр(НСтр.Ндс*Коэффициент/(НСтр.количествокм*СписокКМ.Количество()),2))),",",".")+"""";
				...
				иначе
					...
					строказ=строказ+""""+стрзаменить(?(НСтр.цена=0,"",xmlstring(НСтр.цена*Коэффициент)),".",",")+""","""+
					стрзаменить(xmlstring(?(НСтр.Ндс=0,"",окр(НСтр.Ндс*Коэффициент/НСтр.количествокм,2))),".",",")+"""";
					Текст.ДобавитьСтроку(строказ);
...
Для Каждого Стр_1 Из Объект.Товары Цикл
	...
	Если Объект.ОтправлятьКМВместоАгрегаций Тогда
		...
		Для Каждого Стр_c Из НСтрок Цикл
			...
			если Стр_c.ТипУпаковки=Перечисления.Тип_КМ_ХамелеонЦРПТ.Упаковка
					И Стр_1.GTIN.ТипУпаковки.Наименование<>"BUNDLE"
					И Стр_1.GTIN.ТипУпаковки.Наименование<>"SET"
					тогда 
									СписокКМ=Новый Массив;
					СписокКМ.Очистить();
					//Былcis=Ложь;
...
;
	...
	//		КонецЕсли;
		...
		Иначе
			...
			Если Сред(Штрих,1,2)="01" И СтрДлина(Штрих)>27 ТОгда
...
Функция Сформировать_254_Корректировка(Thumbprint_1,ХэшСумма)
...
ИначеЕсли НЕ Объект.ОтгрузкаНеУчастника Тогда
	...
	Чтение.ЗаписатьНачалоЭлемента("change_property");
	Чтение.ЗаписатьАтрибут("action_id","254");
...
//	конецесли;
	...
	Чтение.ЗаписатьНачалоЭлемента("action_id_type");
	Чтение.ЗаписатьТекст("415");
	Чтение.ЗаписатьКонецЭлемента();
	...
	Чтение.ЗаписатьНачалоЭлемента("details");
...
Если Объект.Исправление=Истина Тогда
	Возврат Сформировать_254_Корректировка(Thumbprint_1,ХэшСумма);
Конецесли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.УдалитьИзУКД.Видимость=ЗначениеЗаполнено(Объект.ТипУточненияУПД);
		Настройки=РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить();
		
		ПроверкаНаСимволКриллицы=Настройки.ПроверкаНаСимволКриллицы;

		ВыделятьНеСовпадениеВОтгрузке=Настройки.ВыделятьНеСовпадениеВОтгрузке;
		Если ВыделятьНеСовпадениеВОтгрузке=Истина тогда
			
//			Для Каждого Стр_1 Из Объект.Товары Цикл
//				Нстр=Объект.СерийныеНомера.НайтиСтроки(Новый Структура("НомерСтрокиСвязи",
//			Стр_1.НомерСтрокиСвязи));
//					Стр_1.ПроверкаСКМ=НСтр.Количество()<>Стр_1.КоличествоКМ;
//			Конеццикла;
Отказ_1=Ложь;
				
				ПроверкаКМов(Отказ_1);
		
					
		Конецесли;
		//	
		//	
		//	УсловноеОформлениеКД=ЭтаФОрма.УсловноеОформление;
		//	ЭлементОформления = УсловноеОформлениеКД.Элементы.Добавить();
		//	
		//	ЭлементОформления.Использование = Истина;
		//	ЭлементОформления.Представление = НСтр("ru = 'Не совпадение с КМ'");
		...
		//	ЭлементОтбора.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Объект.Товары");
	...
	Если Объект.ТекущийПользователь.СайтыВходаВСистему.Наименование="Фармацевтика" Тогда
		...
		Элементы.ОтправлятьКМВместоАгрегаций.Видимость=Ложь;
	...
	Запрос.Текст = 
		...
		|	ИсторияПоCIS_ХамелеонЦРПТ.НомерАгрегата
		|ИЗ
	...
	Пока Выборкаcis.Следующий() Цикл
		Если Не ЗначениеЗаполнено(Выборкаcis.НомерКороба) 
			...
			Стр_Д.НомерАгрегата=Выборкаcis.НомерАгрегата;
	...
	МодульПриСозданииДокументов_ХамелеонЦРПТ.Отгрузка_ХамелеонЦРПТ_ПриСозданииНаСервере(ЭтаФорма,Отказ,СтандартнаяОбработка);
КонецПроцедуры
...
//	КонецЕсли;
	...
	Если (ТипУпаковки=Истина  ИЛИ ТипУпаковки=2)
		И
...
Исключение
	...
	СформироватьФайлДляОтправки=Чтение.Закрыть();
		...
		СформироватьФайлДляОтправки,КодировкаТекста.UTF8,Ложь));
				...
				Подписанный="";
				ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,СформироватьФайлДляОтправки ,Thumbprint_1,Подписанный,ЛОжь);
...
Процедура РекурсияПодсчета_Суммы(Стр_2,КоличествоКМ,РезультатЗапроса)
	...
	Иначе
		Для Каждого Стр_3 Из Стр_2.ПолучитьЭлементы() Цикл
			Если Стр_3.НомерАгрегата<>Стр_2.НомерКМ Тогда
				Сообщить("По КМ "+Стр_3.cis+" не правильный номер агрегата ");
			Конецесли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого Стр_1 Из ДеревоКМ.ПолучитьЭлементы() Цикл
		...
		Добав.НДС=стр_1.НДС;
		Добав.НомерАгрегата=Стр_1.НомерАгрегата;
...
Процедура  РекурсияСохранить(Стр_1,ТекущийОбъект)
	...
	Для Каждого Стр_2 Из Стр_1.ПолучитьЭлементы() Цикл
		...
		Добав.НДС=Стр_2.НДС;
		Добав.НомерАгрегата=Стр_2.НомерАгрегата;
...
Если ЗначениеЗАполнено(Thumbprint_1.АдресПрокси) Тогда
	...
	//	HTTPЗапрос.УстановитьТелоИзСтроки(Тело);
		...
		Подписанный="";
		ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,Текст1,Thumbprint_1,Подписанный,ОтправлятьВЭДООткрепленнуюПодпись,Истина);
...
Процедура  ДобавимЧисловое(МассивПроудкций)
	Для Каждого Стр_1 Из Объект.Товары Цикл
						
		Попытка
			
			Если ЗначениеЗаполнено(Стр_1.GTIN)
				И Число(Строка(Стр_1.GTIN))=Стр_1.GTIN.GTIN Тогда
				
				Если МассивПроудкций.Найти(xmlString(Стр_1.GTIN.GTIN))=Неопределено тогда
					МассивПроудкций.Добавить(xmlString(Стр_1.GTIN.GTIN));
				КонецеслИ;
...
Процедура ОбновитьИнформациюПоФармац(Thumbprint_1)
	...
	МассивПроудкций=Новый Массив;
					...
					//Конеццикла;
					ДобавимЧисловое(МассивПроудкций);
...
Процедура ОтправитьВЭДОСБ_Массив_Черновик(МассивНакладных,ИдентификаторДляОтправки=Неопределено) Экспорт
	...
	//	HTTPЗапрос.Заголовки.Вставить("X-API-Version",	"2.3.1");
		...
		//	ТТТ_Ответ_Сертификат=РаботаСЭДО_На_Клиенте_ХамелеонЦРПТ.ЗарегистрироватьСертификатПоПользователю(Thumbprint_1);
			...
			Возврат;
			Подписанный="";
			ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,ТТТ_Ответ.result.Этап[0].Вложение[0].Файл.Хеш ,Thumbprint_1,Подписанный,ОтправлятьВЭДООткрепленнуюПодпись,Истина);
			...
			МаркаКМ=Стр_М.cis;
				...
				Пока Выборкаcis.Следующий() Цикл
					...
					Пока ВыборкаНомерКороба.Следующий() Цикл
						...
						Добав.НомерАгрегата=ВыборкаНомерКороба.НомерАгрегата;
		...
		EndIf;
			...
			Если NNN.Attachments.Количество()>0 Тогда
					...
					Если ИмяФайла=СтрокаXML.ИмяФайла Тогда
						...
						Thumbprint_1,SignatureBase64,ОтправлятьВЭДООткрепленнуюПодпись, Истина);
...
Функция СоздатьИсправлениеДляОтгрузкиНаСервере()
	...
	Если ЗначениеЗаполнено(Объект.Order_ID_УПД)
		ИЛИ Объект.ТекущийПользователь.СайтыВходаВСистему.Наименование="Фармацевтика"
		тогда
...
Процедура ПроверкаКМов(Отказ)
	
	ТаблС=Объект.СерийныеНомера.Выгрузить();
	ТаблС.Свернуть("НомерСтрокиСвязи");
	
	Для Каждого Стр Из Объект.Товары Цикл
		Стр.ПроверкаСКМ=Ложь;
	Конеццикла;
	
	Для Каждого Стр Из ТаблС Цикл
		
		СтрТ=Объект.Товары.НайтиСтроки(Новый структура("НомерСтрокиСвязи",Стр.НомерСтрокиСвязи));
		НСтр=Объект.СерийныеНомера.НайтиСтроки(Новый Структура("НомерСтрокиСвязи",Стр.НомерСтрокиСвязи));
		КоличествоКМ=0;
		ГТИН=Неопределено;
		Для Каждого Стр_Т Из СтрТ Цикл
			ГТИН=Стр_Т.GTIN;
			КоличествоКМ=КоличествоКМ+Стр_Т.КоличествоКМ;
		Конеццикла;
		Если не ЗначениеЗаполнено(ГТИН) тогда
			ГТИН=НСтр[0].НаименованиеТовара;
		Конецесли;
		Если КоличествоКМ<>НСтр.Количество() Тогда
			Сообщить("В строке с продукцией "+СокрЛП(ГТИН)+" идет не совпадение по количеству "+Строка(КоличествоКМ)+" в табличный части Товары с количеством КМ "+
			строка(НСтр.Количество())+" из серийных номеров" );
			//Отказ=Истина;
			
			Для Каждого Стр_Т Из СтрТ Цикл
				Стр_Т.ПроверкаСКМ=Истина;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	// Вставить содержимое обработчика.
ВыделятьНеСовпадениеВОтгрузке=	ВыделятьНеСовпадениеВОтгрузке();
	...
	Если ВыделятьНеСовпадениеВОтгрузке=Истина тогда
		...
		ПроверкаКМов(Отказ);
			...
			//		Стр_1.ПроверкаСКМ=НСтр.Количество()<>Стр_1.КоличествоКМ;
			//		
			//		Если Стр_1.ПроверкаСКМ=Истина Тогда
			//			
			//			Сообщить("В строке с продукцией "+СокрЛП(Стр_1.GTIN)+" идет не совпадение по количеству "+Строка(Стр_1.КоличествоКМ)+" в табличный части Товары с количеством КМ "+
			//			строка(НСтр.Количество())+" из серийных номеров" );
			//			Отказ=Истина;
			//			
			//		Конецесли;
			//		
			//Конеццикла;
			//Если Объект.Товары.Итог("КоличествоКМ")<>Объект.СерийныеНомера.Количество() Тогда
			//	Сообщить("Не совпадает количество КМ "+Строка(Объект.Товары.Итог("КоличествоКМ"))+" в табличной части Товары с количеством КМ"+
			//	Строка(Объект.СерийныеНомера.Количество())+"в табличной части серийных номеров");
			//	Отказ=Истина;
			//Конецесли;
...
&НаСервереБезКонтекста
Функция ВыделятьНеСовпадениеВОтгрузке()
			Настройки=РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить();
		Возврат Настройки.ВыделятьНеСовпадениеВОтгрузке;
КонецФункции
&НаКлиенте
...
Если Thumbprint_1.НаименованиеСтенда="Фармацевтика" Тогда
		...
		Если Не ЗначениеЗаполнено(Срегистра) 
			ИЛИ Не ЗначениеЗаполнено(СформироватьФайлДляОтправки)
			Тогда
			Возврат;
		КонецЕсли;
...
&НаКлиенте
Процедура УдалитьИзУКДПриИзменении(Элемент)
	// Вставить содержимое обработчика.
	Для каждого Стр Из Объект.СерийныеНомера Цикл
		
		Если Найти(УдалитьИзУКД,Стр.cis)=1 Тогда
			
			НСтр=Объект.Товары.НайтиСтроки(Новый Структура("НомерСтрокиСвязи",Стр.НомерСтрокиСвязи));
			
			НДСЗа1Штуку=Нстр[0].НДС/НСтр[0].КоличествоКМ;
			
			НСтр[0].КоличествоКМ=НСтр[0].КоличествоКМ-1;
			НСтр[0].НДС=НДСЗа1Штуку*НСтр[0].КоличествоКМ;
			НСтр[0].Сумма=НСтр[0].Цена*НСтр[0].КоличествоКМ;
	...
	//Нстр=Объект.СерийныеНомера.НайтиСтроки(Новый Структура("cis",УдалитьИзУКД));	
КонецПроцедуры


&НаКлиенте
Процедура аа_1() Экспорт
	УдалитьИзУКД="";
	
	ЭтаФорма.ТекущийЭлемент=Элементы.УдалитьИзУКД;
КонецПроцедуры

&НаКлиенте
Процедура УдалитьИзУКДАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	// Вставить содержимое обработчика.
	Если СтрдЛина(Текст)>=18 Тогда
		УдалитьИзУКД=Текст;
		УдалитьИзУКДПриИзменении(Элемент);
	Конецесли;
КонецПроцедуры


&НаКлиенте
Процедура ПроверкаКоличествоКМСКМ(Команда)
	// Вставить содержимое обработчика.
Отказ_1=Ложь;
...
Процедура ЗаполнитьВсемиОстаткамиНаСервере()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.ОстаткиКМ_в_Обороте_ХамелеонЦРПТ.Остатки(, Организация = &Организация) КАК ОстаткиКМ_в_Обороте_ХамелеонЦРПТОстатки
		|
		|СГРУППИРОВАТЬ ПО
	...
	МаксимальныйНомерСтрокиСвязиЧисло=1;
	Объект.Товары.Очистить();;
	Пока ВыборкаПродукция.Следующий() Цикл
		...
		Пока ВыборкаСерийныйНомер.Следующий() Цикл
			...
			Если ВыборкаСерийныйНомер.КоличествоОстаток<=0  Тогда
				Продолжить;
			Конецесли;
			//ЗаполнитьИнфуПоШтриху(ВыборкаСерийныйНомер.СерийныйНомер,ДанныеИзПользователя),Сигналить);
			cis=ВыборкаСерийныйНомер.СерийныйНомер;
			Продукция=ВыборкаСерийныйНомер.Продукция;
