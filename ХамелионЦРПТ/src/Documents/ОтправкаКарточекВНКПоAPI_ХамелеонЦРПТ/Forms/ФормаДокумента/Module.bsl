Процедура ЗапишемСтатусВПродукцию(Стр_Ошиб,СтатусК)
...
Процедура ТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Номенкл=Неопределено;
	Если ТекД<>неопределено тогда
		Номенкл=ТекД.Продукция
	Конецесли;
	ОткрытьФорму("Справочник.Продукция_ХамелеонЦРПТ.Форма.ФормаВыбораМножество",
	Новый Структура("ТекущаяСтрока",Номенкл),,,,,Новый ОписаниеОповещения("ПослеВыбораТоваров",ЭтаФорма));
						...
						Если 		NNN["result"]<>Неопределено Тогда
							...
							Если результат["signed"]<>Неопределено Тогда
								Для Каждого Стр_Ошиб Из результат["signed"] Цикл
									ЗапишемСтатусВПродукцию(Стр_Ошиб,ПредопределенноеЗначение("Перечисление.СтатусыКарточекВНК_ХамелеонЦРПТ.published"));
								конеццикла;
...
&НаКлиенте
Процедура ОтправитьНаМодерацию()
	...
	ВернемДляРегистрацииЧерновика=ВернемДляМодерации_ИД(СписокПродукцией);
	 Для каждого Стр_111 Из ВернемДляРегистрацииЧерновика Цикл
		...
		Если значениеЗаполнено(КлючПодключения ) тогда
			...
			ЗапросТекст="v3/feed-moderation?apikey="+Thumbprint_1.КлючAPIККМТ+"&good_id="+Формат(Стр_111,"ЧГ=");
			HTTPЗапрос=Новый HTTPЗапрос(ЗапросТекст);
		Иначе
			ЗапросТекст="v3/feed-moderation?good_id="+Формат(Стр_111,"ЧГ=");
			HTTPЗапрос=Новый HTTPЗапрос(ЗапросТекст );
			HTTPЗапрос.Заголовки.Вставить("Content-Type","application/json;charset=UTF-8");
			HTTPЗапрос.Заголовки.Вставить("Authorization","Bearer "+СРегистра);
		Конецесли;
		...
		Если Ответ.КодСостояния=200 Тогда
			...
			Если NNN["result"]<>Неопределено тогда
				...
				Если результат["error"]<>Неопределено Тогда
					...
					Добав=Объект.ОШибки.Добавить();
					Добав.GTIN=Стр_111;
					Добав.Ошибки=результат["error"];
						
					
					
				КонецеслИ;
				
				Если результат["good_id"]<>Неопределено 
					Тогда
					
						ЗапишемСтатусВПродукцию(результат["good_id"],ПредопределенноеЗначение("Перечисление.СтатусыКарточекВНК_ХамелеонЦРПТ.moderation"));
...
&НаСервереБезКонтекста
Функция ВернемДляМодерации_ИД(СписокПродукцией)
	МассивВозвращаем=Новый Массив;
	Для Каждого Стр Из СписокПродукцией Цикл
		Если ЗначениеЗаполнено(Стр.ГТИН.feed_id) Тогда
				МассивВозвращаем.Добавить(Стр.ГТИН.feed_id);
...
Процедура ОтправитьВЦРПТ(Команда) Экспорт
	...
	Если Объект.ТипОтправки=ПредопределенноеЗначение("Перечисление.ТипОтправкиКарточекВНКПоAPI_ХамелеонЦРПТ.Модерация") Тогда
		ОтправитьНаМодерацию();
		ЭтаФорма.Записать();
		ЭтаФорма.Прочитать();
