...
Процедура ОтправитьПоСУЗ_СервереОБУВ(Thumbprint_1,Адрес,Порт,Знач Защищ,Тело_Ответ)
	Если Защищ<>Неопределено Тогда
		Защищ=Новый ЗащищенноеСоединениеopenSSL();
	КонецЕслИ;
			Попытка
					СРегистра=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.CertKEyНасервере(Thumbprint_1);	
				исключение
					СРегистра=Неопределено;
				КонецПопытки;
				Если НЕ ЗначениеЗАполнено(СРегистра) тогда
...
КонецЕсли;
	...
	Текст1=Ответ.ПолучитьТелоКакСтроку();
		...
		Тело=СтрЗаменить(Тело,Символы.ВК,"");
				...
				HTTPЗапрос=Новый HTTPЗапрос("businessOrders/create");
	//	HTTPЗапрос=Новый HTTPЗапрос("api/orders");
...
КонецПроцедуры
&НаКлиенте
Процедура ОтправитьОбувьПоСУЗ(Thumbprint_1)
	//Thumbprint_1=ПолучитьИзПользователя();	
...
//	Тело=Подписанный;
	...
	Если ОбработкаWEBНаСервере Тогда
		ОтправитьПоСУЗ_СервереОБУВ(Thumbprint_1,Адрес,Порт,Защищ=Неопределено,Тело_Ответ);
...
&НаСервере
Процедура ПечатьПоВыделеннымНомерамКМНаСервере(Таб)
	// Вставить содержимое обработчика.
...
Процедура ПечатьПоВыделеннымНомерамКМ(Команда)
	...
	ПечатьПоВыделеннымНомерамКМНаСервере(Таб);
	...
	//Элементы.ЦЭМ.Видимость=Объект.СпособИзготовленияСИ=Перечисления.СпособИзготовленияСИ_ХамелеонЦРПТ.ЦентрЭтикетированияИМаркировки;
	  ...
	  Если Не ЗначениеЗаполнено(Объект.СпособВыпускаТоваров) Тогда
		  Если ЗначениеЗаполнено(объект.ТекущийПользователь.ТипВводаВОборот) ТОгда
			  Если Объект.ТекущийПользователь.ТипВводаВОборот=Перечисления.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ВводВОборотИмпорт Тогда
...
Функция ТоварыGTINПриИзмененииНаСервере(Товар)
	...
	ИначеЕсли Не ЗначениеЗаполнено(Товар.СтранаПроизводства) тогда
		Если Объект.ТекущийПользователь.ТипВводаВОборот=Перечисления.ТипыДокументаВводВОборот_ХамелеонЦРПТ.ВводВОборот Тогда
...
&НаКлиенте
Процедура ПечатьЭтикеток(Команда) Экспорт
...
КонецПопытки;
	...
	//КонецЕсли;
	 ...
	 ОтправитьОбувьПоСУЗ(Thumbprint_1);
	...
	ММ=СначалGTINБезПод();	
	Если ММ.Количество()>0 Тогда
		ОбновлямДатуПубликации(ММ);
	КонецЕсли;
	
	
	ММ=СначалGTINБезПод();	
	Если ММ.Количество()>0 И 	РежимРаботы()=1 	
  Тогда
		ПоказатьПредупреждение(,"Продукция: "+ММ[0]+" еще не опубликована в ЦРПТ");
		Возврат;
	КонецЕсли;
	...
	Попытка
	///Если НеПрошлаПроверка21(СформироватьФайлДляОтправки) Тогда
...
КонецПопытки;
	...
	Если ОбработкаWEBНаСервере Тогда
		Если НЕ ЗагрузканаСервереОтправитьВРЦПТ(Thumbprint_1,СРегистра,УжеОтправили,Тело) Тогда
				Если УжеОтправили=Истина Тогда
				//ОчиститьТокенСрегистра(Thumbprint_1.Thumbprint_1)
			Иначе
				ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.Получение_Токена(Thumbprint_1);
...
//Для Каждого Стр Из Объект.Товары Цикл
		...
		Если Объект.ТипОплаты=Перечисления.ТипОплаты_ХамелеонЦРПТ.ОплатаПоНанесению Тогда
...
//				Чтен.ЗаписатьЗначение(3);
			...
			Если Объект.ТипПродукции=Перечисления.ТипПродукции_ХамелеонЦРПТ.Парфюм 
				ИЛИ Объект.ТипПродукции=Перечисления.ТипПродукции_ХамелеонЦРПТ.Одежда
				ИЛИ Объект.ТипПродукции=Перечисления.ТипПродукции_ХамелеонЦРПТ.Фото
				Тогда
				Чтен.ЗаписатьИмяСвойства("cisType");
...
Функция ЗагрузканаСервереОтправитьЗаявка_Обувь(Thumbprint_1,Тело_Ответ,Адрес,Порт,Защищ)
...
КонецЕсли;
	...
	NNN=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗаполнитьСтруктуруИзОтветаJSON(Текст1);	
		...
		Подписанный="";
		ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,NNN.orderInfoInBase64,Thumbprint_1.Thumbprint_1,Подписанный,Ложь,Истина);
...
Процедура ОчиститьПодключениюКСУЗНаСервере(Ссылка)
	// Вставить содержимое обработчика.
	РегДв=РегистрыСведений.ТокенДинамичкийДляСУЗ_ХамелеонЦРПТ.СоздатьНаборЗаписей();
...
&НаКлиенте
Процедура ПодписатьИОтправить(Thumbprint_1)
...
//	Тело=Подписанный;
	...
	Если ОбработкаWEBНаСервере Тогда
		ЗагрузканаСервереОтправитьЗаявка_Обувь(Thumbprint_1,Тело_Ответ,Адрес,Порт,Защищ);
...
КонецЕсли;
	...
	NNN=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗаполнитьСтруктуруИзОтветаJSON(Текст1);	
		...
		Подписанный="";
		ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,NNN.orderInfoInBase64,Thumbprint_1.Thumbprint_1,Подписанный,Ложь,Истина);
	...
	Возврат;
		...
		Тело=СтрЗаменить(Тело,Символы.ВК,"");
		Подписанный="";
		ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,Тело,Thumbprint_1.Thumbprint_1,Подписанный,Ложь);
...
Иначе						
			...
			ЭтаФорма.Записать(Новый Структура("РежимЗаписи",РежимЗаписиДокумента.Проведение));
								...
								Если ТипЗнч(Стр_1)=Тип("Структура") ТОгда
										...
										Если Стр_1.errorCode=1160 Тогда
...
&НаСервере
Процедура ПечатьНомеровКМПоВыделеннымТоварамНаСервере(Таб)
	// Вставить содержимое обработчика.
	Мак=Документы.ЗаказМарок_ХамелеонЦРПТ.ПолучитьМакет("Макет");
	ОблМ=Мак.ПолучитьОбласть("Строка|Шапка");
	ПервыйРисунок=Истина;
...
Процедура ПечатьНомеровКМПоВыделеннымТоварам(Команда)
	...
	ПечатьНомеровКМПоВыделеннымТоварамНаСервере(Таб);
	Таб.ТолькоПросмотр=Истина;
	Таб.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КМ";
...
СтруктураНастроек=Неопределено;                                                    СерийныйНомер="";
	...
	Для каждого Стр_1 Из Объект.КМИзAPI Цикл
	   ...
	   Если НаСервере Тогда
		...
		NN=ВернутьТелоЗапросаПодключение(Стр_1.НомерКМ);
...
Иначе
	...
	cis=Стр_1.НомерКМ;
	cis=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.РасКодСтр(cis);
	
	
	
	//cis=Сред(cis,1,Найти(cis,Символ(29)));
	...
	HTTPЗапрос=Новый HTTPЗапрос("api/v3/facade/identifytools/"+cis+"?cis="+cis+"&pg="+Что);
//		HTTPЗапрос=Новый HTTPЗапрос("api/v3/facade/marked_products/info?cis="+cis+"&pg="+что);;
...
&НаКлиенте
Процедура ЗакрытьЗаказ(Команда) Экспорт
	// Вставить содержимое обработчика.
	ЭтаФорма.Модифицированность=Истина;
