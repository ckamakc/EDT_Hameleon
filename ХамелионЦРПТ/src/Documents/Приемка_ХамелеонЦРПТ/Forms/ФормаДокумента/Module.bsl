...
Процедура      ПродолжимДальшенаКлиенте(HTTPСервисЗапрос,Thumbprint_1,СерийныйНомер,СРегистра )
			...
			//	HTTPЗапрос.УстановитьТелоИзСтроки(Тело);
				...
				Подписанный="";
				ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,Текст1,Thumbprint_1.Thumbprint_1,Подписанный,,Истина);
...
Процедура ПринятьПоКонДи(Thumbprint_1)
	...
	Request.WaitForResponse();
						...
						Thumbprint_1.Thumbprint_1,SignatureBase64,, Истина);
...
subjectIdФармацевтика=вернутьSUB();
	...
	Для Каждого Стр_1 Из Объект.СерийныеНомера Цикл
		Если СтрДлина(Стр_1.cis)=18 
			И Объект.Короба.НайтиСтроки(Новый Структура("НомерКороба",Стр_1.cis)).Количество()=0 
			Тогда
			РекурсияУпаковки_Лекарства(СРегистра,Thumbprint_1,Стр_1.cis,Вернутьsub());
		КонецЕсли;
	Конеццикла;
...
&НаКлиенте
Процедура РекурсияУпаковки_Лекарства(СРегистра,Thumbprint_1,cis_1,subjectIdФармацевтика) Экспорт
...
&НаКлиенте
Процедура РекурсияПоПолучениюИнформации_1(Карта,УжеЗапрашивали,Марка,HTTPСервисЗапрос,Thumbprint_1,СРегистра,БылиУпаковки,Стр_11,МассивКМ)
	
	что=Thumbprint_1.Что;
	//Если Карта<>Неопределено И Карта["childrenMap"]<>Неопределено Тогда
	//	//Если не ЗначениеЗаполнено(Стр_11.ТипУпаковки) Тогда
	...
	//	//	КонецЕсли;
	//		
	//	Если Стр_К.Значение<>Неопределено И (Стр_К.Значение["childrenMap"]<>Неопределено 
	//		 ИЛИ Стр_К.Значение["children"]<>Неопределено )
	//		Тогда
	//			ДобавК.ТипУпаковки=ПредопределенноеЗначение("Перечисление.Тип_КМ_ХамелеонЦРПТ.Упаковка");
	//			
	//			МассивКМ.Добавить(Новый Структура("КМ,Тип",ДобавК.ШтрихКод,ДобавК.ТипУпаковки));
	//			
	//			РекурсияПоПолучениюИнформации_1(Стр_К.Значение,УжеЗапрашивали,ШтрихКод,HTTPСервисЗапрос,Thumbprint_1,СРегистра,БылиУпаковки,Стр_11,МассивКМ)
	//		Иначе
	//			
	...
	Если Карта<>Неопределено И Карта["children"]<>Неопределено Тогда
		...
		//	Если Объект.Короба.НайтиСтроки(Новый Структура("НомерКороба,ШтрихКод",Марка,ШтрихКод)).Количество()=0 Тогда
			...
			Иначе
				...
				Для каждого Стр_Х ИЗ            Стр_К["children"] Цикл
					РекурсияПоПолучениюИнформации_1(Стр_Х,УжеЗапрашивали,ШтрихКод,HTTPСервисЗапрос,Thumbprint_1,СРегистра,БылиУпаковки,Стр_11,МассивКМ)
				КонецЦикла;
...
Процедура ЗакачатьДетальнуюИнформацию(Команда)    Экспорт
	...
	Для Каждого Стр Из Объект.Товары Цикл
		...
		Нстр=ОБъект.СерийныеНомера.НайтиСтроки(Новый Структура("НомерСтрокиСвязи",Стр.НомерСтрокиСвязи));
		Для Каждого Стр_М Из НСТр Цикл
			РекурсияУпаковки_ИтогоВложений(Стр_М.cis,Стр.ИтогоВложенийПоСтроке);
		КонецциклА;
	КонецЦикла;	
...
&НаКлиенте
Процедура РекурсияУпаковки_ИтогоВложений(Стр_М,КоличествоНомеровКМ)
	...
	Иначе
		Для Каждого Стр_М_1 Из НСТр Цикл
			РекурсияУпаковки_ИтогоВложений(Стр_М_1.ШтрихКод,КоличествоНомеровКМ);
		КонецциклА;	
	Конецесли;
		...
		//		БылиУпаковки=Ложь;
				Если Ответ.КодСостояния=200 Тогда
					...
					МассивКМ=Новый Массив;
							...
							КонецЕсли;	
							//Если NNN[Стр_11.Марка]<>Неопределено И NNN[Стр_11.Марка]["childrenMap"]<>Неопределено Тогда
