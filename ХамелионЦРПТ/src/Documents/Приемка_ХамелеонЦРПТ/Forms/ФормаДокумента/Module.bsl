...
Если Объект.ОбратныйАкцепт Тогда
	...
	Если (Не ЗначениеЗаполнено(Объект.СхемаОбратногоАкцепта) И Объект._702Схема<>Истина)
		ИЛИ Объект.СхемаОбратногоАкцепта=Перечисления.СхемыОбратногоАкцепта._416Схема 
...
///		Иначе
	...
	ИначеЕсли Объект.СхемаОбратногоАкцепта=Перечисления.СхемыОбратногоАкцепта._442Схема
		 	Тогда
	...
	//Иначе        ////442 схема
		...
		Чтение.ЗаписатьНачалоЭлемента("receive_unregistered_order");
		Чтение.ЗаписатьАтрибут("action_id","442");
		Чтение.ЗаписатьНачалоЭлемента("subject_id");
		Чтение.ЗаписатьТекст(Объект.ДоговорОрганизации.РегистрационныйНомер);
		Чтение.ЗаписатьКонецЭлемента();
...
Процедура ЗакачатьДетальнуюИнформацию(Команда)    Экспорт
	...
	Для Каждого Стр Из Объект.Товары Цикл
		...
		КонецциклА;
	КонецЦикла;	
	ПолучитьНазванияПоGTIN(Команда);
	Если Не ЗначениеЗаполнено(Объект.ТипЭДО) тогда
		Для Каждого Стр Из Объект.Товары Цикл
			Стр.КоличествоНомеровКМ=Стр.ИтогоВложенийПоСтроке;
		Конеццикла;
	Конецесли;
...
&НаКлиенте
Процедура ОбновитьНазванияПоВсемGTIN(Команда)
	// Вставить содержимое обработчика.
	// Вставить содержимое обработчика.
	//ЭтаФорма.Записать();
...
//		Если 
Попытка
Если ЗначениеЗаполнено(Стр_1.Продукция) И Число(Стр_1.GTINНаименованиеТовара)=0 Тогда
	Продолжить;
КонецЕсли;
Исключение
Продолжить;
