...
КонецЕсли;
	...
	Для Каждого Стр_1 Из Объект.СерийныеНомера Цикл
		...
		Если Объект.Короба.НайтиСтроки(Новый Структура("НомерКороба",Стр_1.cis)).Количество()>0
			ИЛИ Объект.Короба.НайтиСтроки(Новый Структура("ШтрихКод",Стр_1.cis)).Количество()>0
			Тогда
			Продолжить;
		КонецЕсли;
		...
		КонецЕсли;
		 ...
		 Конецпопытки;  
			...
			Иначе
				...
				МассивБыл=Новый Массив;
				
				cisИщем=Стр_2.НомерКороба;
				
				Пока Истина Цикл
					
					Если Массивбыл.Найти(cisИщем)<>Неопределено Тогда
						Прервать;
					Конецесли;
					
					Нстрок=Объект.СерийныеНомера.НайтиСтроки(Новый Структура("cis",cisИщем));
					МассивБыл.Добавить(cisИщем);
					
					Если НСтрок.Количество()=0 Тогда
						
						НСтр_Короб=ОБъект.Короба.НайтиСтроки(Новый Структура("ШтрихКод",cisИщем));
						
						Если НСтр_Короб.Количество()=0 Тогда
							Прервать;
						Иначе
							cisИщем=НСтр_Короб[0].НомерКороба;
						Конецесли;
					Иначе
						НСтрок_Товары=Объект.Товары.НайтиСтроки(Новый структура("НомерСтрокисвязи",НСтрок[0].НомерСтрокиСвязи));
						Если НСтрок_Товары.Количество()>0 Тогда
							
							 Идентификатор1=НСтрок_Товары[0];
					
							 КоличествоКМ=Идентификатор1.КоличествоНомеровКМ;
							 Если Не ЗначениеЗаполненО(КоличествоКМ) Тогда
								 КоличествоКМ=Идентификатор1.КоличествоКМ;
							 КонецЕсли;
							 Если ЗначениеЗаполнено(КоличествоКМ) тогда
							 	Добав_Пр.цена=Идентификатор1.Цена*Идентификатор1.КоличествоКМ/КоличествоКМ;
							КонецЕсли;
							 Если ЗначениеЗаполнено(КоличествоКМ) тогда
							 	Добав_Пр.НДС=Идентификатор1.НДС/КоличествоКМ;
							КонецЕсли;
...
//						Добав.Нет=Ложь;
		...
		Для Каждого Стр_11 Из Объект.ПроверкаПоНомерамКМ Цикл
			
			Если Не ЗначениеЗаполнено(Стр_11.Цена) тогда
