...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ЭтоТОбъект.Статус=перечисления.СтатусыДокументов_ХамелеонЦРПТ.Обработан
		...
		Если УКД Тогда
			   ...
			   Для Каждого Стр Из СерийныеНомера1 Цикл
		...
		ИначеЕсли ОбратныйАкцепт=Истина Тогда
...
//	 Заблокировать();
 Исключение
	...
	Для Каждого Стр Из Товары Цикл
		...
		НСтр=СерийныеНомера.НайтиСтроки(Новый Структура("НомерСтрокиСвязи",Стр.НомерСтрокиСвязи));
		Если НСтр.Количество()=0 
			ИЛИ ЭтотОбъект.ТекущийПользователь.СайтыВходаВСистему.Наименование="Фармацевтика"
			Тогда
		...
		Иначе
			Для Каждого Стр_1 Из НСтр Цикл
				СуммаДокумента=СуммаДокумента+Стр_1.Цена;
			Конеццикла;
	...
	Если СерийныеНомера.Количество()>0 тогда
		...
		ИначеЕсли ВсеПриняты=Истина 
			...
			Тогда
			Если Не ЗначениеЗАполнено(СтатусУПД) Тогда
