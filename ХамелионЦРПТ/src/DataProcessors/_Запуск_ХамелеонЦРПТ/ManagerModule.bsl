...
Функция ПродукцияДляДетскогоПитания(V_WEB_90001683) Экспорт
	...
	Если Сред(V_WEB_90001683,1,1)="<" Тогда
	 ...
	 Наименование=СокрЛП(Сред(КодВида_1,Найти(КодВида_1,">")+1));
	 СпрВидОбуви=Справочники.ПродукцияДляДетскогоПитания_ХамелеонЦРПТ.НайтиПоКоду(КодВида);
...
Функция ПолучитьИзПользователя(ТекущийПользователь,ТекСайт="") Экспорт
	...
	Если ТекущийПользователь.ПолучитьТокен=Перечисления.ТипПолученияТокенаИзПользователя_ХамелеонЦРПТ.ИзПараметра 
		...
		ПараметрыДлякилента.Вставить("БратьСертификатНасервере",РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ПолучитьТокенНаСервере
		ИЛИ РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ОбработкаWEBНаСервере
		);
...
Иначе
		ПараметрыДлякилента.Вставить("АдресДляПолученияТокена","markirovka.demo.crpt.tech");
	...
	ПараметрыДлякилента.Вставить("Таймаут",РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ТаймаутОжиданияОтСайта);
	...
	ПараметрыДлякилента.Вставить("ТаймаутОжиданияПослеПлученияКМ",РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ТаймаутОжиданияПослеПлученияКМ);
