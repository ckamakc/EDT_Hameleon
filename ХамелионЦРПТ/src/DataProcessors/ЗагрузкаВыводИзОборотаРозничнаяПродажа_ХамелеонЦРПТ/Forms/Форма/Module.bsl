...
Процедура ОбновитьСтатусДокумента_чек(Текст1,Пользователь)
	...
	ДокумС.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.RETAIL;
...
ДокументПриемки.ПричинаВыводаИзОборота=Перечисления.ПричинаВыводаИзОборота_ХамелеонЦРПТ.RETAIL;
...
//	HTTPЗапрос.УстановитьТелоИзСтроки(Тело);
	...
	Если Ответ.КодСостояния=202 ИЛИ Ответ.КодСостояния=200 Тогда
			...
			Если Thumbprint_1.НаименованиеСтенда="Альтернативный табак" 
				ИЛИ 
				Thumbprint_1.НаименованиеСтенда="Табак" 
				Тогда
				ТекстЗапроса="api/v3/true-api/receipt/listV2?pg="+Что;
			Иначе
				ТекстЗапроса="api/v3/facade/receipt/listV2?pg="+Что;
