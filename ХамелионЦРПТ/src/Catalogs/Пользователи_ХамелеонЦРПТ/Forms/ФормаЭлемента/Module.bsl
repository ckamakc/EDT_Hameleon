...
Процедура ОчиститьПриемкуНаСервере()
	...
	Если ЗначениеЗАполнено(Объект.Ссылка) тогда
		...
		Регдв.Отбор.Организация.Установить(Объект.Организация);
...
Функция ПолучитьСайт()
	Если Константы.РежимРаботыХамелеонЦРПТ.Получить()=2 Тогда
		...
		Возврат "https://markirovka.sandbox.crpt.tech";
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.АдресРегистрацииУчтенойСистемы) Тогда
		Объект.АдресРегистрацииУчтенойСистемы=ОБъект.Организация.АдресМестонахождения;
	Конецесли;
		...
		Если Не ЗначениеЗаполнено(ТекущийОБъект.АдресСайтаСУЗ) тогда
			...
			Константы.РежимРаботыХамелеонЦРПТ.Получить()=1 Тогда
					  ...
					  Иначе
		                  ...
		                  ТекущийОБъект.АдресСайтаСУЗ="https://suz.sandbox.crpt.tech"
...
&НаКлиенте
Процедура ЗарегистрироватьУчетнуюСистему(Команда)
	// Вставить содержимое обработчика.
	Thumbprint_1=ПолучитьИзПользователя();
	...
	Если Thumbprint_1.ПараметрРаботы="Тестовый" Тогда
		...
		Если NNN=Неопределено Тогда
			...
			ПоказатьПредупреждение(,"Не удалось зарегистрировать учетную систему");
		...
		Если Ответ.КодСостояния=200 Тогда
			...
			ИначеЕсли  NNN.status="SUCCESS" Тогда
				ОБъект.omsConnection=NNN.omsConnection;
				ЭтаФорма.Записать();
			 Конецесли;
		...
		Конецесли;
		 Конецесли;
	Конецесли;
		//Если Ответ.КодСостояния=200 И NNN.status="SUCCESS" Тогда
		//	ОБъект.omsConnection=NNN.omsConnection;
		//	ЭтаФорма.Записать();
		//Иначе
		//	Сообщить(Текст1);
...
&НаСервере
Функция ОтправленЗапросНаПолученияИдентификатораУстройстваНаСервере()
	// Вставить содержимое обработчика.
	Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.ИдентификаторСУЗ,"Запрос на получения идентификатора устройства")).Файл;
	Если Хр<>Неопределено тогда
		Возврат Хр.Получить();
		
	Конецесли;
	Возврат "";
	//Возврат Регистрысведений.ОтправленныеПолученные_ХамелеонЦРПТ
КонецФункции

&НаКлиенте
Процедура ОтправленЗапросНаПолученияИдентификатораУстройства(Команда)
	Текст=ОтправленЗапросНаПолученияИдентификатораУстройстваНаСервере();
	ТТ=Новый ТекстовыйДокумент;
	ТТ.ДобавитьСтроку(Текст);
	ТТ.Показать();
	
КонецПроцедуры

&НаСервере
Функция ОтветНаЗапросПолученияИдентификатораУстройстваНаСервере()
	// Вставить содержимое обработчика.
	Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.ИдентификаторСУЗ,"Ответ на запрос на получения идентификатора устройства")).Файл;
	Если Хр<>Неопределено тогда
		Возврат Хр.Получить();
...
&НаКлиенте
Процедура ОтветНаЗапросПолученияИдентификатораУстройства(Команда)
	Текст=ОтветНаЗапросПолученияИдентификатораУстройстваНаСервере();
	ТТ=Новый ТекстовыйДокумент;
	ТТ.ДобавитьСтроку(Текст);
	ТТ.Показать();
КонецПроцедуры
...
&НаСервере
Функция ОтправленЗапросНаПолученияДинамическийТокенНаСервере()
	// Вставить содержимое обработчика.
	Если РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ПолучениеТокенаСУЗЧерезИдентификаторРазработчика Тогда
		Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.omsConnection,"Запрос на получения динамического токена")).Файл;
	Иначе
		Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.ИдентификаторСоединения,"Запрос на получения динамического токена")).Файл;
	Конецесли;
	Если Хр<>Неопределено тогда
		Возврат Хр.Получить();
...
&НаКлиенте
Процедура ОтправленЗапросНаПолученияДинамическийТокен(Команда)
	Текст=ОтправленЗапросНаПолученияДинамическийТокенНаСервере();
	ТТ=Новый ТекстовыйДокумент;
	ТТ.ДобавитьСтроку(Текст);
	ТТ.Показать();
...
&НаСервере
Функция ЛогОтветаНаСервере()
	// Вставить содержимое обработчика.
	Если РегистрыСведений.НастройкиПрограммы_ХамелеонЦРПТ.Получить().ПолучениеТокенаСУЗЧерезИдентификаторРазработчика Тогда
		Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.omsConnection,"Ответ на запрос на получения динамического токена")).Файл;
	Иначе
		Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.ИдентификаторСоединения,"Ответ на запрос на получения динамического токена")).Файл;
	Конецесли;
	Если Хр<>Неопределено тогда
		Возврат Хр.Получить();
...
&НаКлиенте
Процедура ПолучитьТокенСУЗ(Команда)
	// Вставить содержимое обработчика.
	
	Thumbprint_1=ПолучитьИзПользователя();
	ДинамиескийТокен=ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодключениеКСУЗ_ХамелеонЦРПТ(Thumbprint_1,Истина);
КонецПроцедуры
...
Процедура PING(Команда)
	...
	ТекстСохранить="GET "+ТекстСохранить;
	...
	Токен=ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодключениеКСУЗ_ХамелеонЦРПТ(Thumbprint_1);
	Если Не ЗначениеЗаполненО(Токен) Тогда
		Сообщить("Не удалось подключиться к СУЗ по идентификатору "+Thumbprint_1.ИдентификаторСУЗ);
		Возврат;
	Конецесли;
	
	
	
	ТекстСохранить=ТекстСохранить+"api/v2/"+Thumbprint_1.СайтКСУЗ+"/ping?omsId="+Thumbprint_1.ИдентификаторСУЗ;
	...
	ТекстСохранить=ТекстСохранить+Символы.ПС+"clientToken:"+Токен;
	
	
	ТекстСохранить=ТекстСохранить+Символы.ПС+"Тело запроса нет";

	
	Если ЗаписыватьДанныеПриОтправкеЗаказа Тогда
	
	ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗапишемВРЕГИСТР_ОтправленныеПолученные_ХамелеонЦРПТ(Thumbprint_1.ИдентификаторСУЗ,
	ТекстСохранить,"Запрос на проверку доступности по PING");
	Конецесли;
	HTTPЗапрос.Заголовки.Вставить("clientToken",Токен);
	Ответ=HTTPСервисЗапрос.Получить(HTTPЗапрос);
	Если ЗаписыватьДанныеПриОтправкеЗаказа Тогда
	...
	ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗапишемВРЕГИСТР_ОтправленныеПолученные_ХамелеонЦРПТ(Thumbprint_1.ИдентификаторСУЗ,
	Ответ.ПолучитьТелоКакСтроку(),"Ответ на запрос на проверку доступности по PING");
	Конецесли;	
	...
	Попытка
		Если Ответ.КодСостояния=200 И NNN["omsId"]=Thumbprint_1.ИдентификаторСУЗ Тогда
			Сообщить("Успешное подключение к СУЗ по идентификатору "+Thumbprint_1.ИдентификаторСУЗ);
...
&НаКлиенте
Процедура ЛогЗапросаPING(Команда)
	Текст=ЛогЗапросаPINGНаСервере();
	ТТ=Новый ТекстовыйДокумент;
	ТТ.ДобавитьСтроку(Текст);
	ТТ.Показать();
КонецПроцедуры
...
&НаСервере
Функция ЛогЗапросаPINGНаСервере()
	// Вставить содержимое обработчика.
		Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.ИдентификаторСУЗ,"Запрос на проверку доступности по PING")).Файл;
	Если Хр<>Неопределено тогда
		Возврат Хр.Получить();
...
&НаКлиенте
Процедура ЛогОтветаPING(Команда)
	// Вставить содержимое обработчика.
	Текст=ЛогОтветаPINGНаСервере();
	ТТ=Новый ТекстовыйДокумент;
	ТТ.ДобавитьСтроку(Текст);
	ТТ.Показать();
КонецПроцедуры
...
&НаСервере
Функция ЛогОтветаPINGНаСервере()
	// Вставить содержимое обработчика.
	
	Хр=РегистрыСведений.ОтправленныеПолученные_ХамелеонЦРПТ.Получить(Новый Структура("Документ,Вид",Объект.ИдентификаторСУЗ,"Ответ на запрос на проверку доступности по PING")).Файл;
	Если Хр<>Неопределено тогда
		Возврат Хр.Получить();
