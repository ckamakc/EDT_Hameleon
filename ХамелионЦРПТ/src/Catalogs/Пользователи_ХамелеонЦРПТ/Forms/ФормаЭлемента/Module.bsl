...
Процедура ЗарегистрироватьУчетнуюСистему(Команда)
	...
	//	ПоказатьПредупреждение(,"Регистрация учетной системы предназначена только для тестового стенда");
		
	//	Возврат;
		Адрес="suz-integrator.sandbox.crpt.tech";
Иначе
		АдресРаздробить=Thumbprint_1.АдресСУЗ;
		Адрес=Сред(АдресРаздробить,Найти(АдресРаздробить,"://")+3);
		Адрес=Сред(Адрес,1,Найти(Адрес,":")-1);
	...
	КонецеслИ;
	Если Не ЗначениеЗАполненО(Объект.АдресРегистрацииУчтенойСистемы) Тогда
		ПоказатьПредупреждение(,"Необходимо указать адрес местонахождения для регистрации учетной системы");
		Возврат;
	Конецесли;
		  ...
		  //    Адрес="intuot.crpt.ru";
		
			Если ЗначениеЗАполнено(Thumbprint_1.АдресПрокси) Тогда
				Прокси=Новый ИнтернетПрокси;
				Прокси.Установить("http",Thumbprint_1.АдресПрокси,Thumbprint_1.ПортПрокси,Thumbprint_1.ПользовательПрокси,Thumbprint_1.ПарольПрокси);
			
				HTTPСервисЗапрос=Новый HTTPСоединение(Адрес,16443,,,Прокси,Thumbprint_1.Таймаут,Защищ);
			Иначе
				HTTPСервисЗапрос=Новый HTTPСоединение(Адрес,16443,,,,Thumbprint_1.Таймаут,Защищ);
			КонецЕсли;

		HTTPЗапрос = Новый HTTPЗапрос("api/v2/integration/connection?omsId="+Thumbprint_1.ИдентификаторСУЗ); //"static/1555676332000/js/main/preload.js");
		HTTPЗапрос.Заголовки.Вставить("Content-Type","application/json;charset=UTF-8");
		HTTPЗапрос.Заголовки.Вставить("X-RegistrationKey","4344d884-7f21-456c-981e-cd68e92391e8");
		...
		Чтен.ЗаписатьЗначение(Объект.АдресРегистрацииУчтенойСистемы);
		Чтен.ЗаписатьКонецОбъекта();
		Тело=Чтен.Закрыть();
		...
		Подписанный="";
		ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,Тело,Thumbprint_1,Подписанный,Истина);
