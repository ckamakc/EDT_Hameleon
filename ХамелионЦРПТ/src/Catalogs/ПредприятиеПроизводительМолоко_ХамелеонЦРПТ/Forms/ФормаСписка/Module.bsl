...
&НаКлиенте
Процедура НайтиПлощадки(Команда)
	...
	Форм.Заголовок="Укажите GUID площадки в реестре РСХН";
	...
	Если ПодключилисьОк(Thumbprint_1,crsfToken,Срегистра,Куки,HTTPСервисЗапрос,СерийныйНомер) Тогда
		...
		КонецЕсли;
			...
			//Тело=ИщемПоГТИН(РР);
				...
				HTTPЗапрос=Новый HTTPЗапрос("rest/dictionaries/vetis/enterprise?catId=230988&q="+РР);
...
Процедура  СоздатьХСНаСервере(NNN)
	Для каждого Стр Из NNN Цикл
		...
		СпрХС=Справочники.ПредприятиеПроизводительМолоко_ХамелеонЦРПТ.НайтиПоКоду(стр.id);
		Если ЗначениеЗаполнено(СпрХС) тогда
			СпрХС=СпрХС.ПолучитьОбъект()
		Иначе
			СпрХС=Справочники.ПредприятиеПроизводительМолоко_ХамелеонЦРПТ.СоздатьЭлемент();
			СпрХС.Код=стр.id;
		Конецесли;
