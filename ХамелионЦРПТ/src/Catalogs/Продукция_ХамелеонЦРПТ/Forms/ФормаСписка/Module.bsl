...
//		НСтр=Сред(НСтр,1,Найти(НСтр,"""")-1);
	...
	//Для Каждого Стр_1 Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		СформироватьФайлДляОтправки,КодировкаТекста.UTF8,Ложь));
				...
				Подписанный="";
		ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,СформироватьФайлДляОтправки ,Thumbprint_1.Thumbprint_1,Подписанный,Истина,,,Thumbprint_1.PIN);
...
Процедура ЗагрузитьИзНКПоAPI(Команда)
	...
	Если ЗначениеЗАполнено(Thumbprint_1.АдресПрокси) Тогда
		...
		Если значениеЗаполнено(КлючПодключения ) тогда
				HTTPЗапрос=Новый HTTPЗапрос("v3/linked-gtins?apikey="+Thumbprint_1.КлючAPIККМТ+"&limit=1000" );
			//Ответ=HTTPСервисЗапрос.ВызватьHTTPМетод("GET",HTTPЗапрос);
			//HTTPЗапрос=Новый HTTPЗапрос("v3/linked-gtins?"+КлючПодключения );
		Иначе
					HTTPЗапрос=Новый HTTPЗапрос("v3/linked-gtins&limit=1000" );
				HTTPЗапрос.Заголовки.Вставить("Content-Type","application/json;charset=UTF-8");
	HTTPЗапрос.Заголовки.Вставить("Authorization","Bearer "+СРегистра);
		...
		Ответ=HTTPСервисЗапрос.ВызватьHTTPМетод("GET",HTTPЗапрос);
			...
			Если значениеЗаполнено(КлючПодключения ) тогда
				//HTTPЗапрос=Новый HTTPЗапрос("v3/linked-gtins?apikey="+Thumbprint_1.КлючAPIККМТ+"&limit=10000" );
	...
	HTTPЗапрос.Заголовки.Вставить("Authorization","Bearer "+СРегистра);
				...
				Для каждого Стр_1 Из NNN.result Цикл
...
Процедура ЗаписатьКарточкуТовара(Стр_1,ГТИН )
	...
	Если Стр_1.Свойство("good_status",Статус) Тогда
		...
		КонецПопытки;
	Конецесли;
	Для Каждого Стр_1 Из Стр_1.good_attrs Цикл
		Если Стр_1.attr_name="Группа ТНВЭД" Тогда
			СпрПр.КодТНВЭДКороткий=Стр_1.attr_value;
		КонецеслИ;
	Конеццикла;
