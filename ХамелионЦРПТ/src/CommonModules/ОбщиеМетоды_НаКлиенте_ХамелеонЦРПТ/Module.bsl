	...
	Если    Thumbprint_1.СУЗПоНовомуДинамическийТокен=Истина Тогда
		///ПОДКЛЮЧАЕММСЯ ПО НОВОМУ
			...
			Если Ответ.КодСостояния=200 Тогда 	
				...
				Имя_вр_1="";
				ПодписатьФайл(Thumbprint_1.Внешняя,Подписываем,Thumbprint_1.Thumbprint_1,Имя_вр_1,Истина,,,Thumbprint_1.PIN);
		...
		Если Ответ.КодСостояния=200 Тогда 	
			...
			Имя_вр_1="";
			СерийныйНомер="";
			ПодписатьФайл(Thumbprint_1.Внешняя,Подписываем,Thumbprint_1.Thumbprint_1,Имя_вр_1,,,СерийныйНомер,Thumbprint_1.PIN);
...
//			HTTPЗапрос=Новый HTTPЗапрос("rest/signed-data");
...
//			HTTPЗапрос.Заголовки.Вставить("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept, Credentials, Set-Cookie");
...
//		
//			СтрукттураОтвет=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗаполнитьСтруктуруИзОтветаJSON(Ответ.ПолучитьТелоКакСтроку());
//			Дата=СтрукттураОтвет.data;
//			
//			
//			Имя_вр_1="";
//			ПодписатьФайл(Thumbprint_1.Внешняя,Дата,Thumbprint_1.Thumbprint_1,Имя_вр_1,,,СерийныйНомер,Thumbprint_1.PIN);
//			
//			Добав=Имя_вр_1;
//			//Добав=ПолучитьСтрокуИздвоичныхДанных(РР);
//			//Добав=КодСтрСервер(Добав);
...
//			
//			Тело = "{
//			|""fingerprint"": """+Thumbprint_1.Thumbprint_1+""",
//			|""inn"": """+Thumbprint_1.ИНН+""",
//			|""signature"": """+Добав+"""
//			|}";
//			
...
//			
//			
//			HTTPЗапрос=Новый HTTPЗапрос("rest/certlogin");
//			HTTPЗапрос.Заголовки.Вставить("Accept","application/json;charset=UTF-8");
//			//HTTPЗапрос.Заголовки.Вставить("Authorization","Bearer "+Срегистра);
//			HTTPЗапрос.Заголовки.Вставить("Origin","https://"+Thumbprint_1.АдресДляПолученияТокена);
//			HTTPЗапрос.Заголовки.Вставить("Referer","https://"+Thumbprint_1.АдресДляПолученияТокена+"/login");
//			
			...
			//	Для Каждого Стр_С ИЗ Стр_С_1.Значение Цикл
				...
				Конецесли;
					...
					Если Стр_С.Значение["ownerInn"]<>Thumbprint_1.ИНН Тогда
						МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,ВОбороте",Стр_С.Ключ,"КМ принадлежит "+Стр_С.Значение["ownerInn"]+", "+
						Стр_С.Значение["ownerName"],,ВОбороте));
					ИначеЕсли Стр_С.Значение["statusEx"]="WAIT_SHIPMENT" Тогда
						МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,ВОбороте",Стр_С.Ключ,"Ожидает ответа от клиента по документу "+
						Стр_С.Значение["lastDocId"],,ВОбороте));
					Иначе
						МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,ВОбороте",Стр_С.Ключ,Стр_С.Значение["status"],,ВОбороте));
					КонецЕсли;
			//	Конеццикла;
			Конеццикла;
		Конецесли;
...
Процедура ПодписатьФайл(Внешняя,Знач ИмяВр,Текущийпользователь,Имя_вр_1,Открепленная=Ложь,Уже64=Ложь,СерийныйНомер="",PIN="",
	...
	Если ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ОбработкаWEBНаСервере_ПолучитьТокен() Тогда
		 ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ПодписатьФайл(Внешняя,ИмяВр,Текущийпользователь,Имя_вр_1,Открепленная,Уже64,СерийныйНомер);
		 Возврат;
	Конецесли;
...
//Если НеПолучать=Ложь Тогда
	...
	СформироватьФайлДляОтправки=Чтение.Закрыть();
		...
		СформироватьФайлДляОтправки,КодировкаТекста.UTF8,Ложь));
				...
				Подписанный="";
				ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,СформироватьФайлДляОтправки ,Thumbprint_1.Thumbprint_1,Подписанный,ЛОжь,,,Thumbprint_1.PIN);
								 ...
								 Если NNN.Свойство("entries",entries) Тогда
									 	...
									 	Для Каждого Стр_С Из entries Цикл
											...
											Если Стр_С.status="in_realization" Тогда
												МассивСтруктур.Добавить(Новый Структура("cis,статус",БылНомерКМ,"Ожидает ответа от клиента по документу. Владелец  "+Стр_С.branch_id));
											Иначе
												МассивСтруктур.Добавить(Новый Структура("cis,статус",БылНомерКМ,Стр_С.status+". Владелец "+Стр_С.branch_id));
											КонецЕсли;
