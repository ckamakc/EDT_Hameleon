&НаКлиенте
Функция ПолученияТокенПоИдентификатору(Thumbprint_1,ИдентификаторСоединения,ЗаписыватьДанныеПриОтправкеЗаказа)
	//ЗаписыватьДанныеПриОтправкеЗаказа=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗаписыватьДанныеПриОтправкеЗаказа();
	...
	Если Thumbprint_1.НаименованиеСтенда="Фармацевтика"  Тогда
			...
			Если Ответ.КодСостояния=200 Тогда 	
				...
				HTTPЗапрос=Новый HTTPЗапрос("api/v1/token/"+ИдентификаторСоединения);
		...
		ТекстСохранения="POST https://"+Thumbprint_1.АдресКАПИ+"/";
		...
		Если Ответ.КодСостояния=200 Тогда 	
			...
			HTTPЗапрос=Новый HTTPЗапрос("api/v3/auth/cert/"+ИдентификаторСоединения);
			
			
			ТекстСохранения=ТекстСохранения+"api/v3/auth/cert/"+ИдентификаторСоединения;
			ТекстСохранения=ТекстСохранения+Символы.ПС+"Заголовки";
		
			//HTTPЗапрос.Заголовки.Вставить("Content-Type","application/json;charset=UTF-8");
			HTTPЗапрос.Заголовки.Вставить("Content-Type","application/json;charset=UTF-8");
			
			ТекстСохранения=ТекстСохранения+Символы.ПС+"Content-Type:application/json;charset=UTF-8";
			
			
		//	HTTPЗапрос.Заголовки.Вставить("Authorization","Bearer " +СРегистра);
			//HTTPЗапрос.Заголовки.Вставить("Host","ismotp.crptech.ru");
			///Константин  
			ТекстСохранения=ТекстСохранения+Символы.ПС+"Тело запроса";
			ТекстСохранения=ТекстСохранения+Символы.ПС+Тело;
			
			//ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗапишемВРегистр_ЗапросПолученияТокнеаСУЗ(ИдентификаторСоединения,ТекстСохранения);
			...
			Если ЗаписыватьДанныеПриОтправкеЗаказа=Истина Тогда
				ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗапишемВРегистр_ЗапросПолученияТокнеаСУЗ(ИдентификаторСоединения,ТекстСохранения);
				ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗапишемВРегистр_ОтветНаПолученияТокенаСУЗ(ИдентификаторСоединения,Ответ.ПолучитьТелоКакСтроку());
			Конецесли;
...
//				Сообщить("Текстовая строка "+ТекстJSON+" не соответствует шаблону JSON");
			...
			КонецПопытки;
			Ош="";
			Если     NNN.Свойство("description",Ош) Тогда
				Сообщить(Ош);
				Возврат "";
			иНАЧЕЕсли     NNN.Свойство("error_message",Ош) Тогда
				Сообщить(Ош);
				Возврат "";
			КонецЕсли;
...
&НаКлиенте
Функция ПодключениеКСУЗ_ХамелеонЦРПТ(Thumbprint_1,Обновить=ложь) Экспорт
	ЗаписыватьДанныеПриОтправкеЗаказа=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗаписыватьДанныеПриОтправкеЗаказа();
	ПараметрыСтарыеДляПодключения=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ПолучитьНовыеПараметрыДляПодключенияКСУЗ_ХамелоенЦРПТ(Thumbprint_1);
	Если ЗначениеЗаполнено(ПараметрыСтарыеДляПодключения.Токен) И ЗначениеЗаполнено(ПараметрыСтарыеДляПодключения.ДатаВремя) 
		И Обновить=Ложь
		Тогда
		//Если ТекущаяДата()-ПараметрыСтарыеДляПодключения.ДатаВремя<=9.8*60*60 Тогда
		//Если ПараметрыСтарыеДляПодключения.ДатаВремя< ТекущаяДата() Тогда
		Если ТекущаяДата()-ПараметрыСтарыеДляПодключения.ДатаВремя<=9.6*60*60 Тогда
			...
			HTTPЗапрос = Новый HTTPЗапрос("api/v2/"+Thumbprint_1.СайтКСУЗ+"/ping?omsId="+Thumbprint_1.ИдентификаторСУЗ);
			HTTPЗапрос.Заголовки.Вставить("Content-Type","application/x-www-form-urlencoded;charset=UTF-8");
			HTTPЗапрос.Заголовки.Вставить("clientToken",ПараметрыСтарыеДляПодключения.Токен);
			...
			Попытка
				Если Ответ.КодСостояния=200 И NNN["omsId"]=Thumbprint_1.ИдентификаторСУЗ Тогда
					Возврат ПараметрыСтарыеДляПодключения.Токен
				Конецесли;
			Исключение
			КонецПопытки;
	...
	Если Thumbprint_1.ПолучениеТокенаСУЗЧерезИдентификаторРазработчика=Истина 
		и Thumbprint_1.ПараметрРаботы="Тестовый"
		Тогда
		/////Через идентификатор разработчика
		
		Если ЗначениеЗаполнено(Thumbprint_1.omsConnection) тогда
			ИдентификаторСоединения=Thumbprint_1.omsConnection;
			 Возврат ПолученияТокенПоИдентификатору(Thumbprint_1,ИдентификаторСоединения,ЗаписыватьДанныеПриОтправкеЗаказа);

		Конецесли;
		
		
		
		Если Не ЗначениеЗаполнено(Thumbprint_1.АдресРегистрацииПечатиКМ) Тогда
			
			Сообщить("Не заполнен адрес регистрации, где будут печататься КМ, в пользователе Хамелеон ЦРПТ!!");
		...
		Чтен.ЗаписатьЗначение(Thumbprint_1.АдресРегистрацииПечатиКМ);
		...
		Подписанный="";
		ОбщиеМетоды_НаКлиенте_ХамелеонЦРПТ.ПодписатьФайл(Thumbprint_1.Внешняя,Тело,Thumbprint_1,Подписанный,
		,,,,,Истина);
		...
		ПолучитьтелоЗапроса=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ПолучитьтелоЗапроса_СУЗ(Thumbprint_1,Подписанный,Тело);
		...
		Если ЗначениеЗАполнено(Thumbprint_1.АдресПрокси) Тогда
			...
			HTTPСервисЗапрос=Новый HTTPСоединение("xn----7sbbamhfc3aanfgcf0aob1c4h3dua.xn--p1ai",,,,Прокси,Thumbprint_1.Таймаут,Новый ЗащищенноеСоединениеopenSSL());
		Иначе
			
			HTTPСервисЗапрос=Новый HTTPСоединение("xn----7sbbamhfc3aanfgcf0aob1c4h3dua.xn--p1ai",,,,,Thumbprint_1.Таймаут,Новый ЗащищенноеСоединениеopenSSL());
		КонецЕсли;
		ТекстСохранить="POST https://xn----7sbbamhfc3aanfgcf0aob1c4h3dua.xn--p1ai/";

		
		ТекстСохранить=ТекстСохранить+Thumbprint_1.WEBSERVICE_Идентификатор;
		
		Запрос=Новый HTTPЗапрос(Thumbprint_1.WEBSERVICE_Идентификатор);
		//"webservice/");
		...
		ТекстСохранить=ТекстСохранить+Символы.ПС+ПолучитьтелоЗапроса;
		#Если ВебКлиент Тогда
			Запрос.УстановитьТелоИзСтроки(ПолучитьтелоЗапроса,КодировкаТекста.UTF8);
		#Иначе
			Запрос.УстановитьТелоИзСтроки(ПолучитьтелоЗапроса,КодировкаТекста.UTF8,ИспользованиеByteOrderMark.НеИспользовать);
		#КонецЕсли
		
		Ответ=HTTPСервисЗапрос.ОтправитьДляОбработки(Запрос);
		
		Если ЗаписыватьДанныеПриОтправкеЗаказа=Истина тогда
			ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗапишемВРегистр_ЗапросИдентификатораУстройства(
			Thumbprint_1.ИдентификаторСУЗ,ТекстСохранить);
			
		
			ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗапишемВРегистр_ОтветИдентификатораУстройства(
			Thumbprint_1.ИдентификаторСУЗ,Ответ.ПолучитьТелоКакСтроку());
		Конецесли;
		NNN=ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗаполнитьСтруктуруИзОтветаJSON(Ответ.ПолучитьТелоКакСтроку());//ПрочитатьJSON(ЧтениеJSON);
		
		success="";
		NNN.Свойство("success",success);

		
		Если Ответ.КодСостояния=200 Тогда
			Если success=Ложь Тогда
				globalErrors="";
				Если NNN.Свойство("globalErrors",globalErrors) 
					И ТипЗнч(globalErrors)=Тип("Структура")
					Тогда
					errorCode=0;
					Если globalErrors.Свойство("errorCode",errorCode) 
						И errorCode=-3
						Тогда
						
						Сообщить("Нарушение использования! Для активации сообщите разработчикам Ваш ИНН.");
						Возврат "";
					ИначеЕсли globalErrors.Свойство("error",errorCode) тогда
						Сообщить(errorCode);
						Ошибка=errorCode;
						Возврат "";
					Конецесли;
			...
			ИначеЕсли  NNN.status="SUCCESS" Тогда
				ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ЗаписатьomsConnection(Thumbprint_1,NNN.omsConnection);
			 	Возврат ПолученияТокенПоИдентификатору(Thumbprint_1,NNN.omsConnection,ЗаписыватьДанныеПриОтправкеЗаказа);
			 Конецесли;
			 //			Возврат NNN.omsConnection
		Иначе
			Возврат "";
		Конецесли;
	...
	ИначеЕсли    Thumbprint_1.СУЗПоНовомуДинамическийТокен=Истина
		ИЛИ Thumbprint_1.НаименованиеСтенда<> "Фармацевтика" 
		Тогда
		///ПОДКЛЮЧАЕММСЯ ПО НОВОМУ
		
		
		 Возврат ПолученияТокенПоИдентификатору(Thumbprint_1,Thumbprint_1.ИдентификаторСоединения,ЗаписыватьДанныеПриОтправкеЗаказа);
...
Функция СписокИсходящихДокументов(Thumbprint_1,Маркер, Номер,датаДок) Экспорт
	...
	Тотал=1;
	Массив = Новый Массив;
	Пока смещение*1<ТОтал Цикл
		
		ПараметрыЗапроса.PagingInfo.Вставить("Offset",Смещение);
		ПараметрыЗапроса.PagingInfo.Вставить("Count", 1);
...
Тогда
			...
			Для Каждого Стр_С Из NNN Цикл
				...
				Если Стр_С_С["ownerInn"]<>Thumbprint_1.ИНН Тогда
					МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,Вобороте,ТипЭмиссии,НаКомиссии",НомерКМ,"КМ принадлежит "+Стр_С_С["ownerInn"]+", "+
					Стр_С_С["ownerName"],ТипУпаковки));
				ИначеЕсли Стр_С_С["statusEx"]="WAIT_SHIPMENT" Тогда
					МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,Вобороте,ТипЭмиссии,НаКомиссии",НомерКМ,"Ожидает ответа от клиента по документу "+
					Стр_С_С["lastDocId"],ТипУпаковки));
				ИначеЕсли ЗначениеЗаполнено(Стр_С_С["agentInn"]) И Стр_С_С["agentInn"]<>Thumbprint_1.ИНН Тогда
					
					МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,Вобороте,ТипЭмиссии,НаКомиссии",НомерКМ,"КМ передан на комиссию "+Стр_С_С["agentInn"]+", "+
					Стр_С_С["agentName"],ТипУпаковки));
				Иначе
					МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,Вобороте,ТипЭмиссии,НаКомиссии",НомерКМ,Стр_С_С["status"],ТипУпаковки));
				КонецЕсли;
		...
		Иначе
			...
			//	Для Каждого Стр_С ИЗ Стр_С_1.Значение Цикл
				...
				Если Стр_С.Значение["status"]="INTRODUCED" Тогда
					ВОбороте=Истина;
				Конецесли;
				Если Стр_С.Значение["agentInn"]<>Неопределено Тогда
					НаКомиссии=Истина;
				Иначе
					НаКомиссии=Ложь;
				...
				ТипЭмиссии=Стр_С.Значение["emissionType"];
					...
					Если ЗначениеЗаполнено(Стр_С.Значение["agentInn"]) И Стр_С.Значение["agentInn"]<>Thumbprint_1.ИНН Тогда
					 МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,Вобороте,ТипЭмиссии,НаКомиссии",Стр_С.Ключ,"КМ передан на комиссию "+Стр_С.Значение["agentInn"]+", "+
						Стр_С.Значение["agentName"],ТипУпаковки,ВОбороте,ТипЭмиссии,НаКомиссии));
					ИначеЕсли Стр_С.Значение["ownerInn"]<>Thumbprint_1.ИНН Тогда
						МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,ВОбороте,ТипЭмиссии,НаКомиссии",Стр_С.Ключ,"КМ принадлежит "+Стр_С.Значение["ownerInn"]+", "+
						Стр_С.Значение["ownerName"],Стр_С.Значение["packType"],ВОбороте,ТипЭмиссии,НаКомиссии));
					ИначеЕсли Стр_С.Значение["statusEx"]="WAIT_SHIPMENT" Тогда
						МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,ВОбороте,ТипЭмиссии,НаКомиссии",Стр_С.Ключ,"Ожидает ответа от клиента по документу "+
						Стр_С.Значение["lastDocId"],Стр_С.Значение["packType"],ВОбороте,ТипЭмиссии,НаКомиссии));
					Иначе
						МассивСтруктур.Добавить(Новый Структура("cis,статус,ТипУпаковки,ВОбороте,ТипЭмиссии,НаКомиссии",Стр_С.Ключ,Стр_С.Значение["status"],Стр_С.Значение["packType"],ВОбороте,ТипЭмиссии,НаКомиссии));
					КонецЕсли;
			//	Конеццикла;
			Конеццикла;
		Конецесли;
		...
		СтруктураНастроек=Неопределено;                                                    СерийныйНомер="";
				...
				КонецЕсли;
							...
							Если ЗначениеЗАполнено(Thumbprint_1.АдресПрокси) Тогда
									...
									HTTPСервисЗапрос=Новый HTTPСоединение(Thumbprint_1.АдресКАПИ,,,,Прокси,Thumbprint_1.Таймаут,Защищ);
								Иначе
									HTTPСервисЗапрос=Новый HTTPСоединение(Thumbprint_1.АдресКАПИ,,,,,Thumbprint_1.Таймаут,Защищ);
								КонецЕсли;
								
										HTTPЗапрос=Новый HTTPЗапрос("oms-api/signin");
	...
	Организация_1=Неопределено;
		АдресРаздробить=ПараметрыДлякилента.АдресСУЗ;
					...
					Если ЗначениеЗАполнено(ПараметрыДлякилента.АдресПрокси) Тогда
							...
							HTTP_ЦРПТ =  Новый HTTPСоединение(ПараметрыДлякилента.АдресКАПИ,Порт,,,Прокси,ТаймаутОжиданияОтСайта,Защищ);

						Иначе
							
							HTTP_ЦРПТ =  Новый HTTPСоединение(ПараметрыДлякилента.АдресКАПИ,Порт,,,,ТаймаутОжиданияОтСайта,Защищ);
						КонецЕсли;	
						...
						Отв=HTTP_ЦРПТ.ОтправитьДляОбработки(HTTPЗапрос);
						...
						Если Отв.КодСостояния=200 Тогда
							...
							HTTPЗапрос.Заголовки.Вставить("Content-Type","application/json;charset=UTF-8");
							HTTPЗапрос.Заголовки.Вставить("cookie",куки);
							Отв=HTTP_ЦРПТ.Получить(HTTPЗапрос);
...
Процедура ПодписатьФайл(Внешняя,Знач ИмяВр,Текущийпользователь_1,Имя_вр_1,Открепленная=Ложь,Уже64=Ложь,СерийныйНомер="",PIN="",
	ПодписываемКарточку=Ложь,ВсяЦепочка=Ложь)	Экспорт
	...
	Если ТипЗнч(Текущийпользователь_1)=Тип("Строка") тогда
		...
		Если ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ОбработкаWEBНаСервере_ПолучитьТокен() Тогда
			 ОбщийМодуль_НаСервере_ХамелеонЦРПТ.ПодписатьФайл(Внешняя,ИмяВр,Текущийпользователь,Имя_вр_1,Открепленная,Уже64,СерийныйНомер,PIN,ПодписываемКарточку,ВсяЦепочка);
			 Возврат;
		Конецесли;
...
Функция СписокВходящихДокументов(Thumbprint_1,Маркер,КачатьСЭДОЗаПоследниеДни) Экспорт
	...
	Тотал=1;
	Массив = Новый Массив;
	Пока Истина ИЛИ смещение*1<ТОтал Цикл
	...
	//		СообщиТекстСообщения;
		...
		//КонецЕсли;
			...
			СтруктураОтвета = ОбщийМодуль_НаСервере_ХамелеонЦРПТ.СтруктураИзJSON(ОтветСтрокой);
			Если СтруктураОтвета.DocumentModel.Количество()=0 Тогда
				Прервать;
			Конецесли;
