...
//++ МВ:Овчинников 12.04.2021  •0
// Возвращает Версию платформы.
//  //https://infostart.ru/public/610960/
// Возвращаемое значение:  подключаемся на прямую по версии
//  Строка - имя COM-класса для работы с 1С:Предприятием 8 через COM-соединение.
//
Функция ИмяCOMСоединителяПоВерсииПлатформы() Экспорт
	СистемнаяИнфо = Новый СистемнаяИнформация;
	ПодстрокиВерсии = СтрРазделить(СистемнаяИнфо.ВерсияПриложения, ".");
	Возврат "v" + ПодстрокиВерсии[0] + ПодстрокиВерсии[1] + ".COMConnector_"+СистемнаяИнфо.ВерсияПриложения;
КонецФункции//-- МВ:Овчинников 12.04.2021
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	Попытка
		//++ МВ:Овчинников 24.12.2020  •0  //https://infostart.ru/public/610960/
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			//НомерВерсии = "8.3.15.1869";
			ИмяCOMСоединителя = ИмяCOMСоединителяПоВерсииПлатформы();//("V83.COMConnector_" + НомерВерсии);
		#Иначе			
			ИмяCOMСоединителя = ИмяCOMСоединителя(); // "V83.COMConnector"
		#КонецЕсли	
		
		COMConnector = Новый COMObject(ИмяCOMСоединителя);
		//-- МВ:Овчинников 24.12.2020
	Исключение
		Информация = ИнформацияОбОшибке();
		СтрокаСообщенияОбОшибке = НСтр("ru = 'Не удалось подключится по соединению %1 к другой программе: %2'");
		
		Результат.ОшибкаПодключенияКомпоненты = Истина;
		Результат.ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщенияОбОшибке,ИмяCOMСоединителя, ПодробноеПредставлениеОшибки(Информация));
		Результат.КраткоеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщенияОбОшибке,ИмяCOMСоединителя, КраткоеПредставлениеОшибки(Информация));
